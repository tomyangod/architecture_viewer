...
function matchWalkStep(card, steps) {
  ...
  for (const step of steps || []) {
    let score = 0;
    if (fileMatches(card.file, step.path)) score += 10;
    if (card.line != null && step.line != null && Number(card.line) === Number(step.line)) score += 5;
...
function muteAwarenessRule(repo, rule) {
  ...
  const prev = readCursor(repo);
  const muted = [...new Set([...(prev.mutedRules || []), String(rule)])];
