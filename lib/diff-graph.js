...
function diffGraphs(base, head) {
  ...
  for (const [key, hEdge] of headEdges) {
    if (!baseEdges.has(key)) addedEdges.push({ key, edge: hEdge });
  }
  for (const [key, bEdge] of baseEdges) {
    if (!headEdges.has(key)) removedEdges.push({ key, edge: bEdge });
  }
...
function formatDiffText(diff) {
  ...
  if (diff.addedEdges.length > 0) {
    ...
    const interesting = diff.addedEdges.filter((e) =>
      ['import', 'extends', 'implements', 'field-type', 'method-return', 'method-param', 'component-props'].includes(e.edge.type));
    for (const e of interesting.slice(0, 30))
      lines.push(`  + ${e.edge.from} --${e.edge.type}--> ${e.edge.to}`);
  ...
  if (diff.violations.length > 0) {
    ...
    for (const v of diff.violations)
      lines.push(`  ⚠ ${v.fromLayer} → ${v.toLayer}: ${v.from} --${v.type}--> ${v.to}`);
    lines.push('');
  }
