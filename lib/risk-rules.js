...
/**
 * Risk rules engine for incremental session reports and repository snapshots.
 * Snapshot inventory is separate from risk findings and has no change attribution.
 */
...
const { siteCoversValue, siteTouchesEnum } = require('./extract/semantic-sites');
...
const FANOUT_EDGE_TYPES = ['import', 'field-type', 'method-param', 'method-return', 'extends'];
 ...
 * Layer-name policy does not itself confirm the endpoints' assigned roles.
...
function applyTeamForbidCrossLayer(findings, diff, headNodes, rules) {
  ...
  for (const edge of archEdges) {
    ...
    for (const rule of forbids) {
      ...
      applyLayerConfirmation(finding, from, to);
 ...
 * @param {object} [opts] - { snapshot: boolean } 快照摸底下所有边都"新"，不提供"本轮新边"语义
 */
function detectCircularImports(findings, headGraph, baseGraph, addedEdges, cap, opts) {
  ...
  // Always use the full repo-relative path: different directories can contain
  // same-named files (e.g. two browser_launcher.py), and a cycle chain built
  // from basenames conflates them into one unreadable string.
  const label = (id) => {
    const n = nodeOf(id);
    return (n && (n.path || n.name)) || id.replace(/^file:/, '');
  };
  ...
  for (const scc of tarjanScc(adj, headFiles)) {
    ...
    const cycleMembers = cycleNodes
      ...
      .filter(Boolean);
    // Structured ring for explain: ordered edges incl. the closing one, plus
    // which edge is new this round (the one that completed the ring).
    // Snapshot 摸底没有"本轮"概念：不标记 isNew/newEdge，避免误导修法。
    const isSnapshotScan = !!(opts && opts.snapshot);
    const ring = [...cycleNodes, u];
    const cycleEdges = [];
    for (let i = 0; i + 1 < ring.length; i++) {
      cycleEdges.push({
        from: nodeOf(ring[i])?.path || ring[i].replace(/^file:/, ''),
        to: nodeOf(ring[i + 1])?.path || ring[i + 1].replace(/^file:/, ''),
        isNew: !isSnapshotScan && ring[i] === u && ring[i + 1] === v
      });
    }
    const newEdge = isSnapshotScan
      ? null
      : {
          from: nodeOf(u)?.path || u.replace(/^file:/, ''),
          to: nodeOf(v)?.path || v.replace(/^file:/, ''),
 ...
 * @param {object} [opts] - { rules, mode: 'incremental' | 'snapshot' } (default: incremental).
 * Snapshot mode derives current evidence from headGraph, ignoring diff/base/impact.
...
function fanoutCounts(edges) {
  const counts = new Map();
  for (const edge of edges) {
    if (!FANOUT_EDGE_TYPES.includes(edge.type)) continue;
    counts.set(edge.from, (counts.get(edge.from) || 0) + 1);
  }
  return counts;
}

function wiredEntityIds(edges) {
  const ids = new Set();
  for (const edge of edges) {
    if (!WIRING_EDGE_TYPES.includes(edge.type)) continue;
    ids.add(edge.from);
    ids.add(edge.to);
  }
  return ids;
...
function applyLayerConfirmation(finding, from, to) {
  const fromSignal = from && from.layerSignal;
  const toSignal = to && to.layerSignal;
  const confirmed = fromSignal === 'config:.av/layers.json' && toSignal === 'config:.av/layers.json';
  finding.fromLayerSignal = fromSignal || null;
  finding.toLayerSignal = toSignal || null;
  finding.layerConfirmation = confirmed ? 'configured' : 'unconfirmed';
  if (confirmed) return;

  finding.severity = SEVERITY.MEDIUM;
  finding.reportOnly = true;
  finding.title = finding.source === 'architecture-rules.yaml'
    ? '团队分层规则审查（分层未确认）'
    : finding.rule === 'layer-skip' ? '层级穿透审查（分层未确认）' : '跨层依赖审查（分层未确认）';
  finding.message = `当前分层推断显示 ${layerLabel(finding.fromLayer)} → ${layerLabel(finding.toLayer)}；分层未经人工配置确认，仅作审查提示，不阻断`;
  finding.suggestion = '请先核对实际职责，并在 .av/layers.json 中确认两端分层，或用具体模块/路径制定依赖契约。仅有层名禁令不代表分层已确认；不能仅凭目录名、导入或结构推断要求改动依赖。';
}

/** Current observations, not violations or evidence of changes since a baseline. */
function collectSnapshotInventory(graph, opts) {
  const policy = resolveRiskPolicy(opts && opts.rules);
  const T = policy.thresholds;
  const wired = wiredEntityIds(graph.edges);
  const fanout = fanoutCounts(graph.edges);
  ...
  for (const node of graph.nodes) {
    if (node.kind === 'external' || node.kind === 'external-package') {
      externalDependencies.push({ id: node.id, name: node.name || node.id, builtin: !!node.builtin });
    }
    if (node.kind === 'file' && node.lineCount > T.large_file) {
      fileSizes.push({
        rule: node.lineCount > T.god_file ? 'god-file' : 'large-file',
        file: node.path,
        lineCount: node.lineCount,
        threshold: node.lineCount > T.god_file ? T.god_file : T.large_file
      });
    }
    if ((fanout.get(node.id) || 0) >= T.high_fanout) {
      highFanout.push({
        rule: 'high-fanout', id: node.id, name: node.name || node.id,
        file: node.path || null, dependencyCount: fanout.get(node.id), threshold: T.high_fanout
      });
    }
    if (ORPHAN_CHECK_KINDS.includes(node.kind) && !wired.has(node.id)) {
      orphanEntities.push({
        rule: 'orphan-entity', id: node.id, name: node.name || node.id,
        kind: node.kind, file: node.path || null
      });
    }
  }

  const observations = (items) => items.filter((item) => !findingExcluded(item, policy));
  return {
    externalDependencies,
    fileSizes: observations(fileSizes),
    highFanout: observations(highFanout),
    orphanEntities: observations(orphanEntities)
  };
}
...
function evaluateRisk(diff, headGraph, baseGraph, impact, opts) {
  ...
  if (mode !== 'incremental' && mode !== 'snapshot') {
    throw new Error(`Unknown risk evaluation mode: ${mode}`);
  }
  const snapshot = mode === 'snapshot';
  if (snapshot) {
    // Reuse the diff engine's edge evidence, not its temporal interpretation.
    // Neither the caller's graph nor its session diff/baseline is modified.
    baseGraph = { nodes: [], edges: [], fingerprint: '', root: headGraph.root, stats: {} };
    diff = require('./diff-graph').diffGraphs(baseGraph, headGraph);
    impact = null;
  }
  const findings = [];
  const policy = resolveRiskPolicy(opts && opts.rules);
  ...
  for (const v of diff.violations) {
    ...
    applyLayerConfirmation(finding, fromNode, toNode);
    findings.push(finding);
  }
  ...
  for (const t of (snapshot ? [] : diff.removedTypes)) {
  ...
  for (const d of (snapshot ? [] : diff.addedExternalDeps)) {
  ...
  const fanoutCount = snapshot ? new Map() : fanoutCounts(diff.addedEdges);
  ...
  const wired = wiredEntityIds(diff.addedEdges);
  for (const t of (snapshot ? [] : diff.addedTypes)) {
    const node = t.node;
  ...
  if (!snapshot && diff.fileLocChanges && diff.fileLocChanges.length) {
  ...
  detectCircularImports(findings, headGraph, baseGraph, diff.addedEdges, T.cycle_cap, { snapshot });
  ...
  if (opts && opts.rules) {
    ...
    if (!snapshot) applyInvariants(findings, diff, headGraph, baseGraph, opts.rules);
  }
  ...
  if (!snapshot) {
    ...
    applyEnumExhaustiveness(findings, diff, headGraph, baseGraph);
    applyExceptionContractDrift(findings, diff, headGraph);
  }
...
function fileContentChanged(baseGraph, headGraph, filePath) {
  ...
  if (b && !h) return true;
  if (!b || !h) return false;
  const contentChanged = compareSourceContentHashes(b, h);
  if (contentChanged !== null) return contentChanged;
 ...
 * Static associations are not test execution or coverage evidence. Only known
 * unchanged import-associated tests can contribute to the report-only HIGH.
 */
function applyBehaviorTestCoupling(findings, diff, impactMap, opts) {
  ...
  // A missing baseline cannot establish that a test changed this session.
  ...
  const baseTests = new Map((baseIdx.files || []).map((f) => [f.path, f]));
  const headTests = new Map((headIdx.files || []).map((f) => [f.path, f]));
  const seen = new Set();
  ...
  for (const c of changes) {
    ...
    const testFiles = [...new Set(hasTestChange ? cover.testFiles : related.map((e) => e.from))];
    const unchangedRelated = related.filter((e) =>
      compareSourceContentHashes(baseTests.get(e.from), headTests.get(e.from)) === false);
    const hasKnownUnchangedTests = related.length > 0 && unchangedRelated.length === related.length;
    const testEvidence = hasTestChange ? 'changed-associated-tests'
      : hasKnownUnchangedTests ? 'unchanged-associated-tests'
        : related.length ? 'unknown-test-change-status' : 'unrecognized-test-mapping';
    const hasImportAssociation = hasKnownUnchangedTests && unchangedRelated.some((e) =>
      e.via === 'import' && (e.confidence === 'high' || e.confidence === 'medium'));
    const testStatus = hasKnownUnchangedTests ? '关联测试文件未改动'
      : related.length ? '关联测试改动状态未知' : '测试关联未识别';
    const testMessage = hasKnownUnchangedTests
      ? '静态关联的测试文件本轮未改动；未验证测试执行或覆盖'
      : related.length ? '关联测试文件的内容比较证据不足，无法判断测试是否跟进'
        : '未识别到测试关联，无法判断测试是否跟进';
    ...
    if (hasTestChange) {
      ...
      if (downstream >= BROAD_IMPACT_THRESHOLD) {
        findings.push({
          ...
          title: '高影响行为变更（关联测试文件有改动）',
          message: `"${who}" 行为指纹变了且下游 ${downstream}，检测到静态关联的测试文件改动；未验证测试执行或覆盖`,
          detail: nearestTest ? `测试: ${nearestTest}` : prodPath,
          file: prodPath,
          line: c.line || null,
          suggestion: '请审查关联测试改动并运行测试，确认是否覆盖新增/删除的调用与 sink。文件改动不代表测试通过；未判定业务对错。',
          testEvidence,
          testFiles,
          family: 'contract',
          reportOnly: true,
    ...
    if (downstream >= BROAD_IMPACT_THRESHOLD) {
      findings.push({
        rule: 'behavior-changed-broad-impact',
        severity: hasImportAssociation ? SEVERITY.HIGH : SEVERITY.MEDIUM,
        title: `高影响行为变更（${testStatus}）`,
        message: `"${who}" 行为指纹变了，波及 ${downstream} 个下游；${testMessage}`,
        detail: nearestTest
          ? `建议查看测试: ${nearestTest}`
          : `静态测试关联未识别（${prodPath}），不代表没有测试`,
        file: prodPath,
        line: c.line || null,
        suggestion: nearestTest
          ? `请更新 ${nearestTest} 覆盖本次行为变化，或确认该高影响改动不需要测试。未判定业务对错。`
          : '请确认相关测试文件及覆盖范围并运行测试；静态关联缺失不代表没有测试。未判定业务对错。',
        testEvidence,
        testFiles,
        family: 'contract',
        reportOnly: true,
    ...
    // Unknown mapping is an analysis limitation, not evidence of absent tests.
    findings.push({
      rule: 'behavior-changed-no-test',
      severity,
      title: `行为变更（${testStatus}）`,
      message: `"${who}" 行为指纹变了；${testMessage}`,
      detail: nearestTest
        ? `最近测试: ${nearestTest}`
        : `静态测试关联未识别（${prodPath}），不代表没有测试`,
      file: prodPath,
...
function suggestForFinding(finding) {
  ...
  if (finding.layerConfirmation === 'unconfirmed') {
    return finding.suggestion || '请先核对实际职责并确认两端分层；当前仅为审查提示，不要求反转依赖。';
  }
  ...
  if (rule === 'circular-import') {
    const ne = finding.newEdge;
    const where = ne && ne.file ? `（${ne.file}${ne.line != null ? ':' + ne.line : ''}）` : '';
    const edgeText = ne ? `本轮新增的 import${where} ${ne.from} → ${ne.to} 闭合了这个环。` : '';
    return (
      `${edgeText}优先断开这条新边：把被依赖方下沉到更低层、抽共享类型/接口，` +
      `或改为函数内延迟 import（需确认运行时初始化顺序）。只把显式 import 换成动态代理不算解耦。`
    );
  }
  if (rule === 'broad-impact') {
    const n = finding.impactDownstream;
    return (
      `这是影响面提醒，不是已证实的缺陷：变更波及 ${n != null ? n : '多个'} 个下游。` +
      `请先核对直接下游清单、跑/补覆盖这些调用方的测试，再决定是否需要拆分职责；` +
      `不要为了消除这条提醒而机械拆代码。`
    );
  }
  ...
  if (rule === 'enum-exhaustiveness') {
    return finding.suggestion
      || '枚举/联合类型新增了成员，但 switch/match/if-else 站点未覆盖。请补分支或确认 default 足够。观察期 report-only，未判定业务对错。';
  }
  if (rule === 'exception-contract-drift') {
    return finding.suggestion
      || '函数新增了抛出类型，沿调用链到入口未见对应 catch/except。请在边界捕获，或确认由框架处理。观察期 report-only，未判定业务对错。';
  }
  return finding.suggestion || '请根据具体问题内容判断。';
}

function fileNodesByPath(graph) {
  const map = new Map();
  for (const n of (graph && graph.nodes) || []) {
    if (n.kind === 'file' && n.path) map.set(n.path, n);
  }
  return map;
}

function filesReferencingEnum(headGraph, enumNode) {
  const files = new Set();
  if (enumNode.path) files.add(enumNode.path);
  const fileId = enumNode.path ? 'file:' + enumNode.path : null;
  const byId = new Map((headGraph.nodes || []).map((n) => [n.id, n]));
  for (const e of headGraph.edges || []) {
    if (fileId && e.type === 'import' && e.to === fileId) {
      const from = byId.get(e.from);
      if (from && from.path) files.add(from.path);
    }
    if (['field-type', 'method-param', 'method-return'].includes(e.type) && e.to === enumNode.id) {
      const from = byId.get(e.from);
      if (from && from.path) files.add(from.path);
    }
  }
...
function enumMemberAdds(diff, headGraph) {
  const out = [];
  const headNodes = new Map((headGraph.nodes || []).map((n) => [n.id, n]));
  for (const mod of diff.modifiedNodes || []) {
    const change = (mod.changes || []).find((c) => c.field === 'enumValues');
    if (!change || !change.added || !change.added.length) continue;
    const node = headNodes.get(mod.id) || (mod.to && mod.to.id ? headNodes.get(mod.to.id) : null);
    if (!node) continue;
    out.push({ node, added: change.added, all: node.enumValues || change.to || [] });
  }
  return out;
}

function applyEnumExhaustiveness(findings, diff, headGraph) {
  const adds = enumMemberAdds(diff, headGraph);
  if (!adds.length) return;
  const files = fileNodesByPath(headGraph);
  for (const { node, added, all } of adds) {
    const refFiles = filesReferencingEnum(headGraph, node);
    const sites = [];
    for (const fp of refFiles) {
      const fn = files.get(fp);
      for (const site of (fn && fn.switchSites) || []) {
        if (!siteTouchesEnum(site, node.name, all)) continue;
        const missing = added.filter((v) => !siteCoversValue(site, v));
        if (!missing.length) continue;
        sites.push({ file: fp, line: site.line, missing, hasDefault: !!site.hasDefault, kind: site.kind });
      }
    }
    if (!sites.length) continue;
    const uncovered = sites.filter((s) => !s.hasDefault);
    const withDefault = sites.filter((s) => s.hasDefault);
    const evidence = (uncovered.length ? uncovered : withDefault).slice(0, 12);
    const severity = uncovered.length ? SEVERITY.MEDIUM : SEVERITY.LOW;
    const finding = {
      rule: 'enum-exhaustiveness',
      severity,
      title: uncovered.length ? '枚举新增成员未被穷尽' : '枚举新增成员仅由 default 兜底',
      message: `"${node.name}" 新增 ${added.join(', ')}，${uncovered.length || withDefault.length} 处 switch/match 未点名覆盖`,
      detail: evidence.map((s) => `${s.file}:${s.line} 缺 ${s.missing.join(',')}${s.hasDefault ? ' (default)' : ''}`).join(' · '),
...
const GENERIC_THROWS = new Set([
  'Error', 'Exception', 'RuntimeException', 'Throwable', 'BaseException',
  'err', 'error', 'e'
]);

function throwConfidence(name, lang) {
  if (!name) return 'low';
  if (GENERIC_THROWS.has(name) || /^['"`]/.test(name)) return 'low';
  if (lang === 'javascript' || lang === 'typescript' || lang === 'vue' || lang === 'svelte') {
    return /Error$|Exception$/.test(name) ? 'medium' : 'low';
  }
  if (lang === 'python' || lang === 'java') return 'high';
  return 'medium';
}
...
/**
 * Whether a catch site is in scope for a call-path node.
 * Module-level catches (no owner) apply to everyone in the file, except
 * loadGuard (require/import fallback) which never counts for file entrypoints.
 * Function-owned catches only apply to that function (or methods of a class node).
 * File entrypoints ignore unrelated function catch-alls (fixes whole-file swallow FN).
 */
function catchInScope(catchSite, pathNode) {
  if (!catchSite || !pathNode) return false;
  const owner = catchSite.owner || null;
  const isFileNode = pathNode.kind === 'file'
    || (pathNode.id != null && String(pathNode.id).startsWith('file:'));
  if (isFileNode) {
    if (owner != null) return false;
    if (catchSite.loadGuard) return false;
    return true;
  }
  if (owner == null) {
    if (catchSite.loadGuard) return false;
  ...
  if (owner === pathNode.name) return true;
  if ((pathNode.kind === 'class' || pathNode.kind === 'interface')
    && Array.isArray(pathNode.methods) && pathNode.methods.includes(owner)) {
    return true;
  }
  return false;
}

function siteCatchesEx(catchSite, exName) {
  if (!catchSite) return false;
  if (catchSite.hasCatchAll) return true;
  return (catchSite.types || []).some((t) => t === exName || t.endsWith('.' + exName));
}

function siteCatchesExTyped(catchSite, exName) {
  if (!catchSite) return false;
  return (catchSite.types || []).some((t) => t === exName || t.endsWith('.' + exName));
}

function catchCoversLine(catchSite, line) {
  if (line == null) return true;
  const start = catchSite.tryStart != null ? catchSite.tryStart : catchSite.line;
  const end = catchSite.tryEnd != null ? catchSite.tryEnd : catchSite.line;
  return line >= start && line <= end;
}

function throwSitesOf(node, exName) {
  if (!node) return [];
  const impl = node.impl || null;
  const sites = (impl && impl.throwSites) || [];
  return sites.filter((s) => s && (s.name === exName || String(s.name).endsWith('.' + exName)));
}

/** @deprecated whole-file swallow — prefer nodeCatches */
function fileCatches(fileNode, exName, pathNode) {
  if (pathNode) return nodeCatches(fileNode, pathNode, exName);
  if (!fileNode) return false;
  for (const c of fileNode.catchSites || []) {
    if (siteCatchesEx(c, exName)) return true;
  }
  return false;
}

/**
 * @param {number|null|undefined} coverLine when set, catch must lexically cover this line
 * @param {{ origin?: boolean }} [opts] origin: catch-all needs lexical cover; without throw lines, ignore catch-all
 */
function nodeCatches(fileNode, pathNode, exName, coverLine, opts) {
  if (!fileNode || !pathNode) return false;
  const asOrigin = !!(opts && opts.origin);
  for (const c of fileNode.catchSites || []) {
    if (!catchInScope(c, pathNode)) continue;
    if (coverLine != null && !catchCoversLine(c, coverLine)) continue;
    if (asOrigin && coverLine == null && c.hasCatchAll) continue;
    if (asOrigin && coverLine == null) {
      if (siteCatchesExTyped(c, exName)) return true;
      continue;
    }
    if (siteCatchesEx(c, exName)) return true;
  }
...
function originCatches(fileNode, origin, exName) {
  const sites = throwSitesOf(origin, exName);
  if (!sites.length) return nodeCatches(fileNode, origin, exName, null, { origin: true });
  return sites.every((s) => nodeCatches(fileNode, origin, exName, s.line, { origin: true }));
}

function walkCallersToEntry(startId, headGraph, callEdges) {
  const callers = [];
  const seen = new Set();
  const queue = [{ id: startId, depth: 0 }];
  while (queue.length) {
    const cur = queue.shift();
    if (!cur || seen.has(cur.id) || cur.depth > 8) continue;
    seen.add(cur.id);
    const node = nodeById(headGraph, cur.id);
    if (node && cur.id !== startId) callers.push(node);
    if (node && (node.layer === 'controller' || node.layer === 'entrypoint')) {
      if (cur.id !== startId) return { callers, reachedEntry: true, entry: node };
    }
    for (const e of callEdges) {
      if (e.to === cur.id && e.from && !seen.has(e.from)) {
        queue.push({ id: e.from, depth: cur.depth + 1 });
      }
    }
  }
  const entry = callers.find((n) => n.layer === 'controller' || n.layer === 'entrypoint') || null;
  return { callers, reachedEntry: !!entry, entry };
}

function applyExceptionContractDrift(findings, diff, headGraph) {
  const changes = diff.behaviorChanges || [];
  if (!changes.length) return;
  const callEdges = (headGraph.edges || []).filter((e) => e.type === CALL_EDGE);
  const files = fileNodesByPath(headGraph);
  const seen = new Set();
  for (const c of changes) {
    const added = c.addedThrows || [];
    if (!added.length) continue;
    const origin = nodeById(headGraph, c.id);
    const lang = (origin && origin.lang) || 'javascript';
    const { callers, reachedEntry, entry } = walkCallersToEntry(c.id, headGraph, callEdges);
    if (!reachedEntry || !entry) continue;
    for (const ex of added) {
      const key = c.id + '|' + ex;
      if (seen.has(key)) continue;
      seen.add(key);
      const pathNodes = [origin, ...callers].filter(Boolean);
      const caught = pathNodes.some((n) => {
        if (n === origin || (origin && n.id === origin.id)) {
          return originCatches(files.get(n.path), n, ex);
        }
        return nodeCatches(files.get(n.path), n, ex);
      });
      if (caught) continue;
      if (entry === origin || (origin && entry.id === origin.id)) {
        if (originCatches(files.get(entry.path), entry, ex)) continue;
      } else if (nodeCatches(files.get(entry.path), entry, ex)) {
        continue;
      }
      const confidence = throwConfidence(ex, lang);
      const finding = {
        rule: 'exception-contract-drift',
        severity: SEVERITY.MEDIUM,
        title: '新异常可能穿透到入口',
        message: `"${c.method || c.name}" 新增抛出 ${ex}，沿 call 边到 ${entry.layer}「${entry.name}」未见捕获`,
        detail: `${c.path || c.id} → ${entry.path} throw ${ex}`,
        file: c.path || null,
        line: c.line || null,
        evidence: [{ throw: ex, entry: entry.path, entryLayer: entry.layer }],
        family: 'contract',
        reportOnly: true,
        confidence
      };
...
module.exports = {
  ...
  applyEnumExhaustiveness,
  applyExceptionContractDrift,
  fileCatches,
  nodeCatches,
  originCatches,
  fileContentChanged,
