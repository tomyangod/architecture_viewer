 ...
 */
function detectCircularImports(findings, headGraph, baseGraph, addedEdges) {
  ...
  // Head adjacency (file → file import edges only).
  const adj = new Map();
  for (const e of headGraph.edges || []) {
    if (e.type !== 'import') continue;
    if (!headFiles.has(e.from) || !headFiles.has(e.to)) continue;
    if (e.from === e.to) continue; // self-import, not a ring
    if (!adj.has(e.from)) adj.set(e.from, new Set());
    adj.get(e.from).add(e.to);
  }

  // Baseline edge set → edges not present at baseline are "new this session".
  ...
  // BFS path from `start` back to `target` using head adjacency.
  ...
  for (const [u, targets] of adj) {
    for (const v of targets) {
      if (count >= CYCLE_FINDING_CAP) return;
      if (baseEdgeKeys.has(u + '|' + v)) continue; // old edge can't open a new ring
      const path = findPath(v, u); // v → … → u closes the ring u→v→…→u
      if (!path) continue;
      const cycleNodes = [u, ...path.slice(0, -1)]; // u, v, … (u already at path end)
      const ringKey = cycleNodes.slice().sort().join('|');
...
function evaluateRisk(diff, headGraph, baseGraph, impact, opts) {
  ...
  // 小文件（基线行数低于此值）的百分比增长没有统计意义，只按绝对行数增量判定，避免噪音
  const GROWTH_REL_MIN_BASE = 100;
  ...
  detectCircularImports(findings, headGraph, baseGraph, diff.addedEdges);
