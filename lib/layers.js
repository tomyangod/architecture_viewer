...
function layerDefinition(id) {
  return LAYER_BY_ID[id] || { ...LAYER_BY_ID.api, id: snake(id), title: `用户配置 · ${q(id)}` };
}
...
function buildLayerNodes(inv, override) {
  ...
  function add(node) {
    ...
    const def = layerDefinition(layer);
    byId.set(id, {
      id,
      layer,
  ...
  (inv.modules || []).forEach((m) => {
    add({
      id: m.id,
      layer: m.layer || (LAYER_BY_ID[m.kind] ? m.kind : classifyKind(m.label)),
      title: chineseTitle(m.label),
      sub: `${m.label}${m.languages && m.languages.length ? ' · ' + m.languages.join(', ') : ''}`,
      _module: true
    });
  });
...
function groupByLayer(nodes, override) {
  const order = [...new Set((override && Array.isArray(override.order) && override.order.length
    ? override.order
    : LAYER_DEFS.map((l) => l.id)
  ).concat(nodes.filter((n) => !LAYER_BY_ID[n.layer]).map((n) => n.layer)))]
    .filter((id) => LAYER_BY_ID[id] || nodes.some((n) => n.layer === id));
...
function sourceEdges(inv, groups) {
  const ids = new Set(groups.flatMap((g) => g.nodes.map((n) => n.id)));
  return (inv.relationships || [])
    .filter((e) => ['import', 'extends', 'implements'].includes(e.type) && e.file &&
      ids.has(snake(e.from)) && ids.has(snake(e.to)))
    .map((e) => `  ${snake(e.from)} -->|"${e.type} · ${q(e.file)}${e.line ? ':' + e.line : ''}"| ${snake(e.to)}`);
}
...
function renderLayeredFlowchart(inv, opts) {
  ...
  if (!groups.length && !(override && override.edges && override.edges.length)) {
    return 'flowchart TB\n  scan_note["扫描说明：未发现可展示的模块或交付文件"]';
  }
  ...
  const edges =
    override && Array.isArray(override.edges) && override.edges.length
      ? override.edges.map((e) => {
          ...
          return `  ${from} -->|"${q(e.label || '用户配置')}"| ${e.to}`;
        })
      : sourceEdges(inv, groups);

  const hasSourceUser = nodes.some((node) => node.id === 'user');
  const hasActor = !hasSourceUser && !!(override && Array.isArray(override.edges) &&
    override.edges.some((edge) => {
      if (typeof edge !== 'string') {
        return ['user', '使用者'].includes(edge.from) || edge.to === 'user';
      }
      return /^\s*user(?:\s*\(\[[^\]]*\]\))?\s*(?:-->|-\.->)/.test(edge) ||
        /(?:-->|-\.->)\s*user\s*$/.test(edge);
    }));
  const edgeBlock = [...(hasActor ? ['  user(["👤 使用者"])'] : []), ...edges].join('\n');
  ...
  const classAssigns = groups
    ...
    .concat(hasActor ? ['  class user actor'] : [])
    .join('\n');
...
> 静态草稿 / static draft：通解生成，按语义自动分层（${layersUsed || '自适应'}），不代表运行时调用或数据流。关系仅来自有界源码导入/继承或显式 \`architecture.layers.json\`；缺失关系不代表不存在，需人工审阅。
