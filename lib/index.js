  ...
  if (resolved.engine === 'llm') {
    ...
    try {
      ...
      const plan = planRefineFromCache(cache, manifest.hash, {
        only,
        diagramsReady: diagramsOnDisk(kitDir)
      });
      if (plan.skipped) {
        skipped = true;
      } else if (plan.considerIncremental && cache && cache.diagrams && !only) {
        const skeleton = generateFiles(scan(repoRoot));
        const { changed } = diffDiagrams(cache.diagrams, skeleton, kitDir, DIAGRAM_FILES);
        if (changed.length > 0 && changed.length < DIAGRAM_FILES.length) only = changed;
      }
    } catch { /* 增量判定失败则回退全量精修 */ }
