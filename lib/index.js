...
function removeUnselectedTemplates(kitDir, selected) {
  ...
  for (const name of DIAGRAM_FILES) {
    ...
    const template = path.join(productRoot(), name);
    if (path.resolve(p) === template) continue;
    // Legacy kits have no ownership receipt: only an exact shipped template is safe to remove.
    let stat;
    try {
      stat = fs.lstatSync(p);
    } catch (err) {
    ...
    let shipped;
    try {
      shipped = fs.readFileSync(template);
    } catch (err) {
      if (err.code === 'ENOENT') continue;
      throw err;
    }
    if (!content.equals(shipped)) continue;
    fs.unlinkSync(p);
    removed.push(name);
  }
  return removed;
...
function generateToDir(repoRoot, kitDir, opts) {
  ...
  if (cache && cache.engine === 'skeleton' && cache.generationKey === key &&
    ...
    const removedTemplates = removeUnselectedTemplates(kitDir, selected);
