...
const FLOWCHART_VISUAL_RULE = [
  ...
  '5. 禁止编造 Docker/k8s/Ingress/Prometheus；没有证据的进程、数据文件和启动关系不画。',
  '6. 子图2 是有证据的「主链路」特写：flowchart LR；节点用 stadium id(["图标 短中文名"])，禁止 <small> 路径与矩形卡；建议不超过 8 节点，不为凑数添加节点或边。',
  '7. 场景标签要和本视图分工：deployment 用有启动配置依据的本地启动链；禁止把子图1 原样缩写，也禁止与 container/component 共用同一套请求处理措辞。'
].join('\n');
...
const HUMAN_STYLE_RULE = [
  ...
  '命名：优先采用源码和项目说明中已证实的中文业务名；不确定时保留真实符号名，不把示例业务对象套到目标仓。',
  '边标签：≤8 个汉字、符合证据的动宾短语；静态导入不能称为运行时调用。禁止无依据的「协作」；禁止把英文方法签名塞进标签；边标签严禁 ()[]{}（mermaid 会当成形状语法导致整图 Syntax error，如「实时推送(SSE/WS)」应写「实时推送 SSE」）。',
...
const C4_FLOWCHART_RULE = [
  ...
  '7. 场景标签要分工：container 描述有证据的进程交互（有 HTTP 才画 HTTP 进出）；component 描述有证据的代码调用或导入，不画运行时 DB；禁止三视图子图2 双胞胎。'
].join('\n');

const C4_HUMAN_RULE = [
  HUMAN_STYLE_RULE,
  'C4 命名：卡片显示名用有仓库证据的中文业务名；禁止纯英文文件名当卡片标题，但无法确认业务时允许真实符号名；文件路径只放 <small>。',
  '边标签是有证据的中文动宾短语，禁止英文方法签名塞进边标签，不套用固定 Web 分层模板。',
  '外部系统只留已确认的独立运行时依赖；框架库与间接依赖不是独立系统，禁止间接依赖清单化，不预设主存储。'
].join('\n');
