...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === '-V' || a === '--version') {
      out.version = true;
    } else if (a === '--dir' || a === '--repo' || a === '--config' || a === '--out' || a === '--name' || a === '--scope' || a === '--archify' || a === '--renderer' || a === '--code' || a === '--rules' || a === '--lines' || a === '--fail-on' || a === '--intent' || a === '--adapter') {
...
function printGenerateResult(r) {
  ...
  if (r.cached) {
    ...
    console.log('缓存命中：源码、配置、引擎与产物内容一致，' + (r.unchangedDiagrams ? r.unchangedDiagrams.length : 6) + ' 张图全部复用（跳过扫描与生成，' + ms + '）');
  } else {
