...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === '-V' || a === '--version') {
      out.version = true;
    } else if (a === '--dir' || a === '--repo' || a === '--config' || a === '--out' || a === '--name' || a === '--scope' || a === '--archify' || a === '--renderer' || a === '--code' || a === '--rules' || a === '--lines') {
...
async function main(argv) {
  ...
  if (cmd === 'session') {
    ...
    if (subcmd === 'report') {
      ...
      // Explicit gate message: exit code 1 means the architecture gate FAILED
      // (high risk), not that the tool crashed. Important under `set -e` / CI.
      if (riskSummary.level === 'high') {
        console.log(`\n⛔ 架构验收门未通过：检测到 HIGH 风险，退出码 = 1。`);
        console.log(`   报告已正常生成（不是工具崩溃）。请先处理上面的红灯再提交；`);
        console.log(`   确认变更符合预期后，运行 arch-viewer session start 刷新基线。`);
        console.log(`   （CI 中可用退出码 1 阻断合并；--fail-on 阈值见文档。）`);
      } else if (riskSummary.level === 'medium') {
        console.log(`\n🟠 架构门提示：有 MEDIUM 风险，退出码 = 0（不阻断），建议审阅后再提交。`);
      } else {
        console.log(`\n✅ 架构验收门通过：退出码 = 0，无 HIGH/MEDIUM 风险。`);
      }
      try {
        require('./pro/funnel').maybePaywallAfterValueMoment({
