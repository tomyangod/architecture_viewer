...
Usage:
  ...
  arch-viewer session report [repo] [--all] [--open] [--renderer auto|archify|builtin] [--rules <file>]
                              [--fail-on high|medium|low|none]
  arch-viewer session walk [repo] [--open] [--json] [--rules <file>]
                              # 本轮变更人工阅读顺序（≤8 行；等价 MCP av_review_walk）
  arch-viewer session awareness mute <rule> [repo]
  arch-viewer session awareness unmute <rule> [repo]
  arch-viewer session awareness status [repo]
                              # 感知层「不用告诉我」：写入 .av/awareness-cursor.json，不影响门禁
...
async function main(argv) {
  ...
  if (cmd === 'session') {
    ...
    if (subcmd === 'report') {
      ...
      let current;
      try {
        current = buildGraph(repo, { calls: true });
      ...
      try {
        ...
        awareness = buildAwareness({ findings, diff, repo, headGraph: current });
    ...
    if (subcmd === 'walk') {
      ...
      if (!resolved.ok) {
        ...
        console.error((resolved.message || 'No baseline found') + ' (exit 4).');
        console.error('先跑: arch-viewer session report 或 av_guard（有 git 时对照 HEAD；无 git 时会补快照）。');
        return EXIT.NO_BASELINE;
      }
      const baseline = resolved.graph;
      const sessionStart = baseline.sessionStartedAt || baseline.cachedAt || null;
      ...
      const { buildReviewWalk, formatReviewWalkText } = require('./review-walk');
      const walk = buildReviewWalk({
        headGraph: current, baseGraph: baseline, diff, findings
      });

      let htmlPath = path.join(avDir, 'session-report.html');
      try {
        fs.mkdirSync(avDir, { recursive: true });
        const { buildAwareness, markAwarenessSeen } = require('./awareness');
        ...
        const html = generateReport({
          baseGraph: baseline, headGraph: current, diff, findings, analyzerStatus, impact,
          repoName: path.basename(repo), sessionStart, analysisCompleteness: completeness, awareness
        });
        const finalized = finalizeSessionHtml({
          repo,
          builtinHtml: html,
          renderer: require('./view-policy').DEFAULT_SESSION_RENDERER
        });
        htmlPath = finalized.htmlPath;
        // walk-only output doesn't present awareness cards to user - don't mark as seen
      } catch (e) {
        console.error(`Scan/report write failed (exit 3): ${e.message}`);
        return EXIT.SCAN_FAILED;
      }
      ...
      if (args.json) {
        ...
        console.log(formatReviewWalkText(walk, { reportPath: htmlPath }));
        if (walk.stats && walk.stats.skipCount) {
          console.log('按阅读顺序打开源码核对越层与副作用；结构全貌见 session-report.html。');
        } else {
          console.log('按阅读顺序打开源码；结构全貌见 session-report.html。');
        }
      }
      if (args.open) {
        openInBrowser(htmlPath);
        console.error('(已在默认浏览器打开 HTML 报告)');
      }
      if (completeness.status === 'failed' || completeness.status === 'incomplete') {
        console.error(`分析${completeness.status === 'failed' ? '失败' : '不完整'}（exit 3）：${(completeness.reasons || []).join('；')}`);
        return EXIT.SCAN_FAILED;
      }
      // walk 是人审辅助，不充当架构门；红灯请用 session report / guard。
      return EXIT.OK;
    }
    if (subcmd === 'guard') {
    ...
    if (subcmd === 'awareness') {
      const { muteAwarenessRule, unmuteAwarenessRule, readCursor } = require('./awareness');
      const action = args._[2];
      if (action === 'status') {
        const awRepo = path.resolve(args._[3] || process.cwd());
        const cursor = readCursor(awRepo);
        const muted = cursor.mutedRules || [];
        console.log(`Checking repo: ${awRepo}`);
        console.log(`mutedRules: ${muted.length ? muted.join(', ') : '(none)'}`);
        console.log(`lastSeen: ${cursor.lastSeenAt || '(never)'}`);
        return 0;
      }
      if (action === 'mute' || action === 'unmute') {
        const rule = args._[3];
        const awRepo = path.resolve(args._[4] || process.cwd());
        if (!rule) {
          console.error('Usage: arch-viewer session awareness mute|unmute <rule> [repo]');
          return EXIT.USAGE_ERROR;
        }
        let cursor;
        try {
          cursor = action === 'mute'
            ? muteAwarenessRule(awRepo, rule)
            : unmuteAwarenessRule(awRepo, rule);
        } catch (e) {
          console.error(`Invalid rule (exit 2): ${e.message}`);
          return EXIT.USAGE_ERROR;
        }
        console.log(`Checking repo: ${awRepo}`);
        console.log(`${action === 'mute' ? '已静音' : '已恢复'}感知规则: ${rule}`);
        console.log(`mutedRules: ${(cursor.mutedRules || []).join(', ') || '(none)'}`);
        console.log('不影响架构门禁退出码。');
        return 0;
      }
      console.error('Usage: arch-viewer session awareness mute|unmute <rule> [repo]');
      console.error('       arch-viewer session awareness status [repo]');
      return EXIT.USAGE_ERROR;
    }
    if (subcmd === 'history') {
    ...
    console.error('Usage: arch-viewer session start|report|walk|awareness|guard|history [repo]');
    return 2;
  }
  ...
  if (cmd === 'workspace') {
    ...
    if (subcmd === 'report') {
      ...
      if (args.json) {
        ...
        return results.some((r) => r.riskSummary &&
          (r.riskSummary.gateLevel || r.riskSummary.level) === 'high') ? 1 : 0;
      }
