...
const { computeImpact, formatImpactText } = require('./impact');
...
async function main(argv) {
  ...
  if (cmd === 'diff') {
    ...
    if (args.json) {
      ...
      const diffFindings = evaluateRisk(diff, headGraph, baseGraph);
  ...
  if (cmd === 'workspace') {
    ...
    if (subcmd === 'report') {
      ...
      for (const r of results) {
        ...
        console.log(`  ${icon} ${name} ${level.padEnd(7)} +${c.added} -${c.removed} ~${c.modified}${c.extAdded || c.extRemoved ? ` 外依+${c.extAdded}-${c.extRemoved}` : ''} ${fText}`);
        if (r.hasChanges) {
