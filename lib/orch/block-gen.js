...
function runOrch(engineTag, repoRoot, opts) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const publicize = (s) => String(s).replace(
      /\[(digest|rich1|orch|nocritic|orch2|orch3|orch4|orch8|auto|edge)\]/g,
      '[精修]'
    );
    child.stdout.on('data', (chunk) => {
      outBuf = cap(outBuf, chunk);
      process.stdout.write(publicize(chunk.toString()));
    });
    child.stderr.on('data', (chunk) => {
      errBuf = cap(errBuf, chunk);
      process.stderr.write(publicize(chunk.toString()));
    });
    child.on('error', (err) => {
...
/**
 * 产品精修默认入口：auto 引擎（按 shape 置信度自动选择）。
 *   shape 判定高置信 → orch4（确定性取证 + 闸门 + 扫尾）
 *   shape 低置信/已知歧义组合 → orch8（Agent explore 兜底降级）
 * 引擎选择明细写入产物目录 engine-choice.json。
 */
async function generateBlockAuto(repoRoot, opts) {
  const out = await runOrch('auto', repoRoot, opts);
...
// 旧名保留兼容（历史调用方）；现在内部就是 auto 引擎
const generateBlockOrch4 = generateBlockAuto;
...
module.exports = { generateBlockAuto, generateBlockOrch4, generateBlockOrch3 };
