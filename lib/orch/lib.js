...
async function chat(messages, opts = {}) {
  ...
  if (!key) {
    const e = new Error('缺少 DEEPSEEK_API_KEY 环境变量');
    e.fatal = true;
    throw e;
  }
...
function gatherExcerpts(root, tree, capChars = 90000) {
  ...
  for (const d of ['changedetectionio/api', 'changedetectionio/blueprint', 'changedetectionio/processors',
    ...
    if (tree.dirs.has(d)) {
      const listing = fs.readdirSync(path.join(root, d))
        .filter((n) => !n.startsWith('__pycache__'))
        .map((n) => n + (fs.statSync(path.join(root, d, n)).isDirectory() ? '/' : '')).join('  ');
...
function parseFlowchart(code) {
  ...
  return { nodeIds: [...nodeIds], edges, subgraphs, undeclared: [...undeclared] };
}
  ...
  while ((m = smallRe.exec(md))) {
    ...
    for (let part of parts) {
      ...
      for (let tok of tokens) {
        tok = tok.replace(/^[./\-]+|[./\-]+$/g, '');  // 只剥首尾点/斜杠/短横，保留 ( ) [ ] 路由段
        if (!tok) continue;
...
function lintBlockDiagram(md, tree) {
  ...
  for (const code of blocks) {
    const { nodeIds, edges, subgraphs, undeclared } = parseFlowchart(code);
...
const DIAGRAM_EXPECTED_HEADER = {
  'c4-context.md': /^C4Context/m,
  'c4-container.md': /^C4Container/m,
  'c4-component.md': /^C4Component/m,
...
function lintDiagram(md, fileName, tree) {
  ...
  for (let bi = 0; bi < blocks.length; bi++) {
    ...
    if (headerRe && !headerRe.test(code)) {
      const expected = Object.keys(DIAGRAM_EXPECTED_HEADER).find((k) => k === fileName);
      issues.push(`${label}: mermaid 头类型错误，${fileName} 应使用 ${expected === 'deployment-ops.md' ? 'flowchart' : expected === 'class-diagram.md' ? 'classDiagram' : code.match(/^C4\w+|^classDiagram|^flowchart|^graph/m) ? '正确的 C4 类型' : '规定的图类型'}`);
    }
  ...
  // 6. 路径核查：节点描述中引用的文件/目录路径必须在代码树中存在
  if (tree && allPathTokens.length) {
    const missing = [];
    const checked = new Set();
    for (const tok of allPathTokens) {
...
function importanceForensics(root, tree, entrypoints) {
  ...
  function info(p) {
    ...
    for (const [kw, reason] of optional) {
      ...
      if (re.test(low)) optReasons.push(reason);
    }
  ...
  for (const d of tree.dirs) {
    ...
    if (/node_modules|vendor|\.git|dist|build|__pycache__|test|docs?|examples?|scripts/.test(d)) continue;
    const i = info(d);
  ...
  const shape = (() => {
    ...
    // ===== 形态信号（统一排除测试/文档/插件/UI 目录，避免 monitor-types/mqtt.js、ReverseProxy.vue 误报）=====
    ...
    // bridge：文件级信号也要算（z2m 协议侧是 lib/mqtt.ts、lib/zigbee.ts，非 mqtt/ 目录）
    ...
    // proxy：reverseproxy/caddyfile 等无歧义词；UI 组件（ReverseProxy.vue）不算
    ...
    // notify-bus：核心 pubsub/投递原语（server/topic.go、server/smtp_server.go）；
    // 排除 generic message.go（listmonk internal/manager/message.go 是邮件编排、memos internal/email 是渲染）；
    // 第二备选不含 subscription——业务实体「订阅」（listmonk internal/core/subscriptions.go 邮件订阅者）
    // 不是 pub/sub 原语；真正的订阅原语只在 server/ 布局下认定（ntfy server/subscription.go）
    ...
    // 检查真实前端（除了 templates 之外的 JSX/Vue/React）
    const realFe = fe || [...tree.files].some(f => /\.(vue|tsx|jsx)$/.test(f)
      && !TEST_ASSETS_PATH.test(f)
      && !/(^|\/)node_modules\//.test(f));
    // 有前端依赖但不是真的有 UI 源：视为无前端
    const noUi = !realFe;

    if (anyBridge && !realFe) return { kind: 'bridge', name: '协议桥接', signal: [...bridgeTokens].join(',') };
    if (anyBridge) return { kind: 'bridge', name: '协议桥接（附前端）', signal: [...bridgeTokens].join(',') };
    if (anyProxy && noUi) return { kind: 'proxy', name: '反向代理/网关' };
    if (anyNotifyBus) {
      return realFe
        ? { kind: 'notify-bus', name: '通知总线（附前端）' }
        : { kind: 'notify-bus', name: '通知总线' };
    }
    if (anyProxy) return { kind: 'proxy', name: '反向代理/网关（附前端）' };
    if (hasCargo && !fe) return { kind: 'api-svc', name: '后端服务（Rust）' };
    if (hasGoMod && !fe) return { kind: 'api-svc', name: '后端服务（Go）' };
    return { kind: 'web', name: 'Web 应用（前后端）' };
  })();
  // 空层建议（按形态推断，plan 阶段应跳过空层，避免为"凑七层"硬画节点）
  const emptyLayers = [];
  // 形态信号强 + 无真实前端 → frontend 层建议空
  ...
  return { info, edges: edges.length, optionalKw: [...optional.keys()], detachedList: detachedList.slice(0, 8), feRoots: fw.feRoots, shape, emptyLayers };
}
...
module.exports = {
  chat, extractJson, chatJson, chatBlock, extractBlockMd, usage, maskKey,
