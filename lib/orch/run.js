 ...
 *   variant: digest | rich1 | orch | nocritic | orch2 | orch3
 *
...
const variant = process.argv[3] || 'orch';
const outdir = path.resolve(process.argv[4] || './out');
...
// 可选/旁路组件标注闸门：图中出现 optional/detached 关键词节点，中文名必须带「可选/实验」
...
const INFRA_RE = /postgres|postgre|mysql|mariadb|redis|clickhouse|mongo|sqlite|elastic|kafka|rabbitmq|nats|数据库|database/i;
...
function structureGates(md, anchors, importance, productTokens) {
  ...
  for (const [id, n] of nodes) {
    ...
    byPath.get(n.path).push(id);
  }
  for (const [p, ids] of byPath) {
    if (ids.length > 1) {
      issues.push({ severity: 'high', kind: 'dup-path',
        issue: `同路径重复节点：${ids.join('、')} 都指向 ${p}`,
        fix: '合并为一个节点：保留主链路上的那个、删除重复声明，所有边改连保留节点；同一文件/目录禁止出现两个节点' });
    }
  }
  ...
  // 5) 机制误标：声称实时推送但路径不是 realtime 锚点（events 事件总线 ≠ websocket）
  // 强文件名豁免：后端源文件名本身就是 sse_hub.go / ws_server.py / websocket_handler.js 时，
  // 文件即推送机制（memos sse_hub.go 与 sse_handler.go 同目录共存，锚点去重只留一个）
  const STRONG_RT_FILE = /(^|[/_-])(sse|ws|websocket|web-socket|socket)[\w.-]*\.(py|js|ts|go|java|rb|php|cs|rs)$/i;
  for (const [id, n] of nodes) {
    if (!n.path) continue;
    if (/实时推送|实时刷新|实时监控|实时更新|realtime|websocket|web[\s-]?socket|\bSSE\b/i.test(n.head)) {
      ...
      if (!ok) {
        // 前端接收端豁免：浏览器里的实时客户端（static/js/realtime.js、socket mixin）本来就是推送的接收方，
        // 仓库存在服务端推送锚点时「实时推送客户端」表述合法（changedetection fe_realtime 教训）
        ...
        issues.push({ severity: 'high', kind: 'mech-mislabel',
          issue: `节点 ${id}（${n.path}）标注为实时推送，但该路径不是 websocket/sse/realtime 推送端点${realtimeAnchors.length ? '；真实实时推送锚点是 ' + realtimeAnchors.join('、') : '（本仓库没有实时推送机制）'}`,
    ...
    for (let pass = 0; pass < 4; pass++) {
      ...
      let relocN = 0;
      {
        ...
        if (sgs0.length) { // 仅子图1 分层全景有 subgraph；特写块跳过
          ...
          for (const [id, n] of bs0.nodes) {
            ...
            if (sg && sg.key && sg.key !== 'storage') misplaced.push({ id, from: sg });
          }
          ...
          if (movables.length) {
            ...
            for (const x of movables) {
              relocN++;
              actions.push(`圆柱存储节点归位：${x.id} 从「${x.from.title}」层移到「数据存储」层并修正 class`);
            }
          }
        }
      }
      ...
      if (isCloseup) {
        ...
        if (longIds.length >= 4 && ![...cn.nodes.keys()].some((id) => /^[A-Z]$/.test(id))) {
          ...
          lines = lines.map((l) => {
            let out = l;
            for (const [longId, s] of shortMap) out = out.replace(new RegExp('\\b' + longId + '\\b', 'g'), s);
            return out;
          });
          shortN++;
...
async function runOrch4() {
  ...
  const shape = importance.shape || { kind: 'web', name: 'Web 应用' };
  const emptyLayers = importance.emptyLayers || [];
...
- 内部节点直连 ext_upstream / ext_ca 等外部系统是合法运行时边，不要绕路`,
    bridge: `- 本产品是协议桥接：核心叙事必须出现双侧协议对端——ext_broker（消息代理）与 ext_device（设备网络），两者都必须出场并连边
...
${emptyLayers.length ? `【空层禁令】本仓库不存在以下层，禁止画（不得为凑层数硬画，也不得把模板/配置文件塞进去凑数）：${emptyLayers.join('、')}` : ''}`;
  const extIdText = extNodes.length
    ? `\n【形态外部系统固定 id】edges 的 from/to 必须逐字使用这些节点 id：${extNodes.map((x) => `${x.id}=${x.name}${x.optional ? '（可选）' : ''}`).join('，')}。
- 所有外部系统节点（含 ext_db）与外部角色同样用 stadium 形状声明在所有 subgraph 之外（禁止放进任何层 subgraph 内），并加 classDef actor 上色；内部节点 --> 外部系统是合法运行时边（代理转发/协议桥接/推送投递都经此外部对端），禁止删除或改道
- 可选外部系统（如 ext_db）出场时用虚线 -.-> 连接并在标签标注「可选」；嵌入式库（SQLite/Bolt 等进程内文件库）不画外部数据库节点`
    : '';
...
- 最多 10 条边`;
  ...
  // 8) 确定性扫尾：LLM 三轮修不掉的机械问题（孤立装饰节点、响应反向边）程序化处理
  {
...
module.exports = { runOrch3, visualGates, optionalLabelGate, notifyDirectionGate, structureGates, deterministicSweep, parseBlockStructure, BLOCK_SPEC };
