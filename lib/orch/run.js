 ...
 * （规划→路径求证→起草→静态校验→批判→修订→校验）显式做成状态机。
 *
 * 用法：
...
const { flattenNestedSubgraphs, sanitizeCylinderDecls } = require('../flowchart-sanitize');
...
5. 箭头必须有中文语义标签：A -->|调用/读写/投递/触发/通知| B；边方向=真实运行时数据流方向；边标签 ≤8 个汉字，用动宾短语（如「拉取HTML」「投递任务」「有变更」「发通知」），禁止写整句话；边标签严禁 ()[]{}（mermaid 会当成 stadium/矩形/菱形语法导致整图 Syntax error，如「实时推送(SSE/WS)」应写「实时推送 SSE」）
...
12. 文件结构：# Block Diagram — 分层模块 + 一句引言（产品是干什么的）+ ## 子图1：分层全景（TB 全图 mermaid 块）+ ## 子图2：<核心动作>主链路（flowchart LR，6–8 个节点，用简短中文名串起默认主流程，节点可不写 path）；
   末尾保留：---  与一行  *由 Architecture Viewer 编排流水线生成 · 已过路径/语法静态校验 · 语义请人工复核*
...
// ---------- 节点 5：静态校验（确定性：路径存在性 + mermaid 语法 + 层归属；不校验运行时语义） ----------
function nodeFactcheck() {
  ...
  state.lint = lint;
  log(`静态校验：幻觉路径 ${lint.metrics.幻觉路径数}，层归属错误 ${state.layerIssues.length}，lint 问题 ${lint.issues.length} 条`,
...
function visualGates(md) {
  ...
  // 4b) 边标签含 ()[]{} —— mermaid 形状保留字符，整图 Syntax error
  const unsafeLabs = findUnsafeMermaidEdgeLabels(main);
  for (const lab of unsafeLabs.slice(0, 5)) {
    const fix = sanitizeMermaidEdgeLabel(lab);
    issues.push({
      severity: 'high',
      kind: 'spec',
      issue: `边标签含 mermaid 形状保留字符 (): 「${lab}」`,
      fix: `改为「${fix || '短中文动宾'}」或去掉括号（否则 viewer 整图 Syntax error 回退占位图）`
    });
  }
...
function deterministicSweep(md, anchors, productTokens) {
  ...
  const fixBlock = (code, blockIdx) => {
    ...
    // 0-pre) 边标签去掉 mermaid 形状保留字符 ()[]{} —— 否则 viewer 整图 Syntax error 回退占位图
    // （舆情 monitor 教训：`|实时推送(SSE/WS)|` → Parse error Expecting STADIUMEND）
    {
      let n = 0;
      lines = lines.map((l) => l.replace(/(-\.?->|---)\s*\|([^|]*)\|/g, (full, arrow, lab) => {
      ...
      if (n) {
        actions.push(
          `边标签去掉 mermaid 形状保留字符 ()[]{}：${n} 处（避免 Syntax error，如「实时推送(SSE/WS)」→「实时推送 SSE/WS」）`
        );
      }
    }
    // 0-pre2) 拆平嵌套 subgraph + 圆柱空格（舆情 Container：APP 套 L_* / id[( "x" )] → Syntax error）
    {
      const before = lines.join('\n');
      let code = flattenNestedSubgraphs(before);
      code = sanitizeCylinderDecls(code);
      if (code !== before) {
        lines = code.split('\n');
        actions.push('flowchart 语法清洗：拆平嵌套 subgraph / 修正圆柱声明空格');
      }
    }
