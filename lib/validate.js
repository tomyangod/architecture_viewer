...
const { DEFAULT_DIAGRAM_FILES } = require('./view-policy');
...
function checkFile(filePath, content, opts) {
  ...
  blocks.forEach((block, i) => {
    ...
    // UpdateLayoutConfig 仅原生 C4 方言需要；flowchart 化的 C4 精修片（classDef/subgraph）不需要
    const nativeC4 = /^\s*C4(?:Context|Container|Component|Dynamic|Deployment)\b/m.test(block.code);
    if (isC4 && nativeC4 && !/UpdateLayoutConfig\s*\(/.test(block.code)) {
...
function listDiagramFiles(dir) {
  ...
  return DIAGRAM_FILES.filter((f) => fs.existsSync(path.join(dir, f)));
}
...
function validateDir(dir, opts) {
  ...
  const results = [];
  const missing = [];
  const filesProvided = Array.isArray(options.files);
  const wanted = filesProvided
    ? options.files.slice()
    : (options.requireAll ? DIAGRAM_FILES.slice() : listDiagramFiles(dir));
  const required = options.requireAll
    ? DIAGRAM_FILES.slice()
    : (wanted.length ? wanted : (filesProvided ? [] : DEFAULT_DIAGRAM_FILES.slice()));
...
function mentionedIds(dir) {
  ...
  for (const f of DIAGRAM_FILES) {
    ...
    for (const raw of text.split(/[^a-zA-Z0-9_]+/)) {
      ...
      if (snake.length >= 3 && snake !== raw.toLowerCase()) {
        ids.add(snake);
      }
    }
  }
