...
const { enumValuesFromEntity } = require('./extract/semantic-sites');
...
/**
 * sys.path-style extra roots (backend/, src/): specifier "services.x"
 * maps to unique "backend/services/x". Never match a single segment
 * (avoids `from json import dumps` binding a stray json.py).
 * Ambiguous suffixes stay unresolved.
 */
function uniquePyModuleSuffix(pyModuleToFile, needle) {
  if (!needle || !pyModuleToFile) return null;
  const parts = String(needle).split('/').filter(Boolean);
  if (parts.length < 2) return null;
  let hit = null;
  for (const [mod, file] of pyModuleToFile) {
    if (mod !== needle && !mod.endsWith('/' + needle)) continue;
    if (hit) return null;
    hit = { module: mod, file };
  }
  return hit;
...
function resolvePyModule(specifier, fe, pyModuleToFile, importedName) {
  ...
  if (importedName && /^[A-Za-z_][A-Za-z0-9_]*$/.test(importedName)) {
    ...
    const bySuffix = uniquePyModuleSuffix(pyModuleToFile, sub);
    if (bySuffix) return { module: bySuffix.module, file: bySuffix.file, kind: 'submodule' };
  }
  ...
  const bySuffix = uniquePyModuleSuffix(pyModuleToFile, modulePath);
  if (bySuffix) return { module: bySuffix.module, file: bySuffix.file, kind: 'module' };
  return null;
}
...
function buildGraph(rootDir, opts) {
  ...
  for (const fe of allFiles) {
    ...
    addNode({
      ...
      switchSites: fe.switchSites && fe.switchSites.length ? fe.switchSites : undefined,
      catchSites: fe.catchSites && fe.catchSites.length ? fe.catchSites : undefined
    });
    ...
    for (const e of fe.entities) {
      addNode({
        ...
        enumValues: enumValuesFromEntity(e)
      });
