...
function buildGraph(rootDir) {
  ...
  for (const fe of allFiles) {
    ...
    addNode({
      ...
      path: fe.file,
      layer: guessLayer(fe.file, fe.lang),
      lang: fe.lang,
...
function guessLayer(filePath, lang) {
  const normalized = filePath.toLowerCase();
  // React components / UI layer (highest presentation level)
  ...
  if (/\/?(repository|repositories|dao|daos|mapper|mappers|persistence|storage|db)\//.test(normalized) ||
