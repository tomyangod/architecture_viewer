 ...
 */
function resolvePyModule(specifier, fe, pyModuleToFile) {
  let modulePath;
  if (specifier.startsWith('.')) {
    ...
    modulePath = allParts.join('/');
  } else {
    modulePath = specifier.replace(/\./g, '/');
  }

  // Try direct module match, then __init__ (package)
  const candidates = [modulePath, modulePath + '/__init__'];
  for (const c of candidates) {
    if (pyModuleToFile.has(c)) {
      return { module: c, file: pyModuleToFile.get(c) };
    }
  }
  return null;
}
...
function buildGraph(rootDir) {
  ...
  for (const fe of allFiles) {
    ...
    addNode({
      ...
      path: fe.file,
      layer: guessLayer(fe.file, fe.lang, userLayers),
      lang: fe.lang,
      lineCount: fe.error ? 0 : undefined
    });
    ...
    if (fe.lang === 'java') {
      ...
      for (const imp of fe.imports) {
        ...
        if (resolved) {
          ...
          // External or stdlib
          const topPkg = modSpec.split('.')[0];
          const isStdlib = isPyStdlib(modSpec);
...
function guessLayer(filePath, lang, userLayers) {
  ...
  if (userLayers && typeof userLayers === 'object') {
    for (const seg of segments) {
      if (userLayers[seg]) return userLayers[seg];
    }
  }

  // Built-in patterns
  if (/\/?(components?|ui|widgets?|screens?|containers?|layouts?)\//.test(normalized) ||
      /\.(component|page|view|screen|layout)\.(tsx|jsx|ts|js)$/.test(normalized)) return 'component';
  ...
  if (/\/?(service|services|usecase|usecases|facade|facades|business|hooks?|store|stores|composables?)\//.test(normalized) ||
      /\.(service|usecase|facade|store|hook)\.(java|js|ts|tsx|vue|svelte|py|go)$/.test(normalized)) return 'service';
  ...
  if (/\/?(repository|repositories|dao|daos|mapper|mappers|persistence|storage|db|database|databases)\//.test(normalized) ||
  ...
  if (/\/?(util|utils|helper|helpers|common|shared|lib|libs|tools?)\//.test(normalized)) return 'util';
  return null;
}
...
module.exports = { buildGraph, extractGraphTo, guessLayer };
