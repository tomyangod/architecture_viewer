...
/** 一键开启 7 天 Pro trial（与 loginFlow 相同：首次登录自动建档）。 */
async function startTrial(opts) {
  console.log('开启 7 天 Pro 试用（首次登录自动建档为 trial）…');
  const user = await loginFlow(opts);
  if (user.entitlement === 'trial' && user.trialUntil) {
    console.log('试用已激活，有效至 ' + String(user.trialUntil).slice(0, 10));
  }
  return user;
}
...
async function requirePro(opts) {
  ...
  try {
    ...
    try {
      require('./funnel').emit('upgrade_required', { reason: 'anonymous', feature: o.feature || null });
    } catch { /* ignore */ }
  ...
  if (!user.active) {
    try {
      require('./funnel').emit('upgrade_required', { reason: 'expired', feature: o.feature || null });
    } catch { /* ignore */ }
