...
function requireProFeature(req, feature) {
  const features = require('./features');
  if (feature && !features.isFeatureEnabled(feature)) {
    const err = new Error(
      'Pro 特性「' + features.featureTitle(feature) + '」已由服务端关闭（ARCH_PRO_FEATURES）。'
    );
    err.status = 403;
  ...
  const user = token ? sessionByToken(token) : currentUser(req);
  if (!user) {
    const err = new Error(features.upgradeMessage({ reason: 'anonymous', feature }));
    err.status = 401;
    err.code = 'UPGRADE_REQUIRED';
    err.payload = features.upgradePayload({ reason: 'anonymous', feature });
    throw err;
  }
  if (!isActive(user).ok) {
    const err = new Error(features.upgradeMessage({ reason: 'expired', feature }));
    err.status = 402;
    err.code = 'UPGRADE_REQUIRED';
    err.payload = features.upgradePayload({ reason: 'expired', feature });
    throw err;
  }
  return user;
}
...
function bearerOrCookie(req) {
  ...
  return parseCookies(req.headers && req.headers.cookie).av_session || null;
}
...
function requestLoginCode(email, ip) {
  ...
  if (createdTrial) {
    store.track('trial', { userId: user.id, until: user.trialUntil, source: 'auth_login' });
  }
...
function verifyLoginCode(email, code) {
  ...
  store.track('login', { userId: user.id, source: 'code' });
  return { token, user: publicUser(user) };
}
