...
const { makeDirSkip } = require('./scan-ignore');
...
function collectClasses(root, budget) {
  const skipped = makeDirSkip(root, SKIP_DIRS);
  const classes = [];
  const stack = [root];
  let files = 0;
  while (stack.length && classes.length < 24 && files < budget) {
    ...
    if (dir !== root && skipped(base, dir)) continue;
    for (const ent of listDir(dir)) {
      const p = path.join(dir, ent.name);
      if (ent.isDirectory()) {
        if (!skipped(ent.name, p)) stack.push(p);
        continue;
      }
...
const CODE_EXT = /\.(py|ts|tsx|js|jsx|mjs|cjs|vue|svelte|java|go|rb|rs|php|cs|kt|swift|c|cpp|h)$/i;

/** Recursively check if a directory contains source code files (depth-limited). */
function hasCodeFiles(dir, depth, skipped) {
  if (depth > 2) return false;
  const skip = skipped || makeDirSkip(dir, SKIP_DIRS);
  for (const ent of listDir(dir)) {
    if (ent.isFile() && CODE_EXT.test(ent.name) && !/test|spec/i.test(ent.name)) return true;
    const child = path.join(dir, ent.name);
    if (ent.isDirectory() && !skip(ent.name, child)) {
      if (hasCodeFiles(child, depth + 1, skip)) return true;
    }
  }
  return false;
}
...
function detectLanguagesFromSource(root, languages) {
  ...
  function walk(dir, depth) {
    ...
    for (const ent of listDir(dir)) {
      ...
      if (ent.isDirectory()) {
        const child = path.join(dir, ent.name);
        if (skipped(ent.name, child)) continue;
        walk(child, depth + 1);
        continue;
      }
...
function collectPythonImports(root, packages) {
  ...
  for (const ent of listDir(root)) {
    ...
    if (!ent.isDirectory() || skipped(ent.name, child)) continue;
    if (fs.existsSync(path.join(child, '__init__.py'))) {
...
function scan(root) {
  ...
  const skipped = makeDirSkip(abs, SKIP_DIRS);
  ...
  function walkCompose(dir, depth) {
    ...
    for (const ent of listDir(dir)) {
      ...
      if (!ent.isDirectory() || skipped(ent.name, nest)) continue;
  ...
  const topDirs = listDir(abs)
    .filter((e) => e.isDirectory() && !skipped(e.name, path.join(abs, e.name)))
    .map((e) => e.name);
  ...
  const MODULE_NOISE = new Set([
    ...
    'docs', 'doc', 'website', 'site', 'examples',
  ...
  const { layers: userLayers } = loadLayerDefinitions(abs);
  function addModule(id, label, dirName, operational = false) {
    if (moduleSet.has(id) || modules.length >= 30) return;
    moduleSet.add(id);
    const configured = matchUserLayer(label, userLayers);
    const layer = configured ? configured.layer : moduleLayer(dirName);
    const moduleLanguages = new Set();
    detectLanguagesFromSource(path.join(abs, label), moduleLanguages);
    modules.push({
      id,
      label,
      path: label,
  ...
  // Operational tools are source modules, but tools/ itself is not an ops unit.
  const OPERATIONAL = /schedul|monitor|alert|alarm|observ|notify|cron|orchestr/i;
  function collectOperational(dir, depth) {
    if (depth > 3 || modules.length >= 30) return;
    for (const ent of listDir(dir)) {
      const child = path.join(dir, ent.name);
      if (ent.isDirectory() && !skipped(ent.name, child) &&
          !/^(tests?|fixtures|vendor|node_modules)$/i.test(ent.name)) {
        const rel = path.relative(abs, child);
        if (OPERATIONAL.test(ent.name) && hasCodeFiles(child, 0, skipped)) {
          addModule(snake(rel), rel, ent.name, true);
        }
        collectOperational(child, depth + 1);
      } else if (ent.isFile() && CODE_EXT.test(ent.name) && OPERATIONAL.test(ent.name)) {
        const rel = path.relative(abs, child);
        addModule(snake(rel), rel, ent.name, true);
        const mod = modules.find((m) => m.path === rel);
        if (mod) mod.languages = [EXT_TO_LANG[path.extname(rel).slice(1)]].filter(Boolean);
      }
    }
  }
  if (!skipped('tools', path.join(abs, 'tools'))) collectOperational(path.join(abs, 'tools'), 0);
  // 1) Top-level dirs: CODEISH match OR contains code files (skip test/docs/examples)
  for (const d of topDirs) {
    if (!isModuleDir(d)) continue;
    if (CODEISH.test(d) || hasCodeFiles(path.join(abs, d), 0, skipped)) {
  ...
  for (const d of topDirs) {
    ...
    if (!CODEISH.test(d) && !hasCodeFiles(topPath, 0, skipped)) continue;
    ...
    for (const ent of listDir(topPath)) {
      ...
      if (!ent.isDirectory() || skipped(ent.name, subPath)) continue;
      if (!isModuleDir(ent.name)) continue;
      if (isGoPkg || isUtilityLeaf(ent.name)) continue;
      if (hasCodeFiles(subPath, 0, skipped)) {
  ...
  if (listDir(path.join(abs, '.github', 'workflows')).some((e) => e.isFile() && /\.ya?ml$/i.test(e.name))) {
    deploy.push('.github/workflows');
  }
  ...
  // Reuse the bounded static extractor; never turn inventory order into dependencies.
  const graph = require('./extract-graph').buildGraph(abs);
  inventory.classes = graph.nodes
    .filter((n) => ['class', 'interface', 'struct', 'component'].includes(n.kind))
    .slice(0, 60)
    .map((n) => ({ id: n.id, name: n.name, file: n.path, language: n.lang }));
  const classIds = new Set(inventory.classes.map((c) => c.id));
  inventory.classRelationships = graph.edges.filter((e) =>
    ['extends', 'implements'].includes(e.type) && classIds.has(e.from) && classIds.has(e.to));
  const fileNodes = new Map(graph.nodes.map((n) => [n.id, n]));
  const owners = modules.slice().sort((a, b) => b.path.length - a.path.length);
  function owner(file) {
    if (!file) return null;
    const mod = owners.find((m) => file === m.path || file.startsWith(m.path + '/'));
    if (mod) return mod.id;
    return entrypoints.includes(file) ? snake(file.replace(/\.[^.]+$/, '')) : null;
  }
  inventory.relationships = uniqueBy(graph.edges
    .filter((e) => ['import', 'extends', 'implements'].includes(e.type))
    .map((e) => ({
      from: owner((fileNodes.get(e.from) || {}).path),
      to: owner((fileNodes.get(e.to) || {}).path),
      type: e.type, file: e.file, line: e.line
    }))
    .filter((e) => e.from && e.to && e.from !== e.to),
  (e) => `${e.from}:${e.type}:${e.to}`);
  inventory.sourceFacts = { bounded: true, stats: graph.stats };
