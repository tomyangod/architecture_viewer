...
function scan(root) {
  ...
  const entrypoints = [];
  for (const f of ['main.py', 'app.py', 'index.html', 'dashboard.html', 'valuation_monitor.py', 'server.js', 'main.go', 'manage.py', 'scheduler.py', 'worker.py', 'Cargo.toml', 'src/main.rs', 'src/lib.rs', 'cli.js', 'index.js', 'package.json']) {
...
function driftTerms(inventory) {
  ...
  const MANIFEST_FILES = new Set(['package.json', 'Cargo.toml']);
...
function checkDrift(inventory, mentioned, haystack) {
  ...
  const missing = [];
  // Treat '-', '_' and whitespace as equivalent: dir "openim-push" often becomes
  // mermaid id "openim_push" in LLM output, and compose service "openim-web" is
  // often rendered in C4 labels as "OpenIM Web" (space-separated English name).
  const termRe = (s) =>
    new RegExp('\\b' + escapeRegex(s).replace(/[-_]/g, '[-_\\s]') + '\\b');
  const variantHit = (v) =>
    v.length >= 3 ? termRe(v).test(hay) : hay.includes(v);
...
module.exports = { scan, snake, driftTerms, checkDrift, hasCodeFiles, escapeRegex, SKIP_DIRS };
