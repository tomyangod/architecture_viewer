...
function archiveTreeish(repo) {
  const gitRoot = findGitRoot(repo);
  if (!gitRoot) return null;
  const rel = path.relative(gitRoot, path.resolve(repo));
  if (!rel || rel.startsWith('..')) {
    return { cwd: gitRoot, treeish: 'HEAD' };
  }
  const posix = rel.split(path.sep).join('/');
  return { cwd: gitRoot, treeish: posix ? `HEAD:${posix}` : 'HEAD' };
}

function scanGitHeadTree(repo) {
  const spec = archiveTreeish(repo);
  if (!spec) {
    return { error: 'SCAN_FAILED', message: '找不到 Git 根，无法导出 HEAD 树。' };
  }
  const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'av-head-'));
  ...
  try {
    ...
    if (arch.status !== 0 || !fs.existsSync(tarPath)) {
      ...
      return { error: 'SCAN_FAILED', message: `导出 git HEAD 失败：${err}` };
    }
    ...
    if (tar.status !== 0) {
      ...
      return { error: 'SCAN_FAILED', message: `解压 git HEAD 失败：${err}` };
    }
    overlayWorkingConfig(repo, extractDir);
    const raw = buildGraph(extractDir);
    const graph = toPersistableGraph(raw);
    graph.testIndex = buildTestIndex(extractDir);
    return { graph };
  } catch (e) {
    return { error: 'SCAN_FAILED', message: `扫描 git HEAD 失败：${e.message}` };
  } finally {
...
function readHeadCache(repo, sha) {
  ...
  if (!cached || cached.gitHead !== sha || !cached.fingerprint) return null;
  return cached;
}
