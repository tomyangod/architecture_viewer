...
const { filterClassesForFocus } = require('./view-policy');
...
function generateFiles(inv, options) {
  const context = `C4Context
    System(repo_system, "${q(inv.title || inv.folder)}", "${q((inv.languages || []).join(', ') || 'unknown')}")
    UpdateLayoutConfig($c4ShapeInRow="3", $c4BoundaryInRow="1")`;
  const classInv = filterClassesForFocus(inv, options && options.focusFiles);
  const all = {
    ...
    'c4-component.md': document('C4 Component — 组件详情', [
      ['模块静态依赖', c4Modules(inv, true)], ['源码类型', classBody(classInv, true)]
    ]),
    'block-diagram.md': blockDiagramMarkdown(inv),
    'class-diagram.md': document('Class Diagram — 代码结构', [['源码类型与继承', classBody(classInv, false)]]),
    'deployment-ops.md': deployment(inv)
  };
  const views = options && Array.isArray(options.views) ? options.views : null;
  if (!views || !views.length) return all;
  const out = {};
