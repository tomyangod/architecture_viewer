...
function walkSignals(node, acc) {
  ...
  if (IF_TYPES.has(node.type)) acc.control.if++;
    ...
    if (tn) {
      acc.throws.push(tn);
      acc.throwSites.push({ name: tn, line: node.startPosition.row + 1 });
    }
...
function extractImpl(fnNode) {
  ...
  if (!body) return null;
  const acc = { tokens: [], literals: [], calls: [], throws: [], throwSites: [], awaits: [], control: emptyControl() };
  ...
  const throwSites = (acc.throwSites || []).slice(0, LIST_CAP);
