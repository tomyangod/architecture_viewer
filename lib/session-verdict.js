 ...
 * @returns {{ level: string, lines: string[], text: string, topFinding: object|null, reportPath: string }}
 */
function formatSessionVerdict(opts = {}) {
  const riskSummary = opts.riskSummary || { level: 'none' };
  const findings = opts.findings || [];
  const summary = opts.summary || {};
  const level = riskSummary.level || 'none';
  const meta = lightMeta(level);
  const reportPath = normalizeReportPath(opts.reportPath);
  const topFinding = level === 'none' ? null : pickTopFinding(findings);

  const line1 = `${meta.icon} ${meta.label} · ${riskPhrase(riskSummary, findings)} · ${changePhrase(summary)}`;
  let line2;
  if (level === 'none') {
