...
const WIRING_EDGE_TYPES = new Set([
  ...
  'method-return', 'component-props', 'di-registered', 'uses', 'calls', 'call'
]);
...
function computeImpact(diff, baseGraph, headGraph) {
  ...
  // W20: behavior fingerprint changes are impact seeds even when structure fields are unchanged.
  for (const c of diff.behaviorChanges || []) {
    if (c && c.id) seeds.push({ id: c.id, change: 'behavior' });
    if (c && c.path) seeds.push({ id: 'file:' + c.path, change: 'behavior-file' });
  }
