...
function getImportedNames(importClause) {
  ...
  // Default import: `import Foo from '...'` -> direct identifier child
  const defaultId = importClause.childForFieldName('name');
  if (defaultId && defaultId.type === 'identifier') names.push(defaultId.text);
  ...
  if (named) {
    for (let i = 0; i < named.childCount; i++) {
      ...
      if (spec.type === 'import_specifier') {
        // field `name` is the imported binding (may be aliased via field `alias`)
...
function extractParamTypes(paramsNode) {
  ...
  for (let i = 0; i < paramsNode.childCount; i++) {
    ...
    if (p.type !== 'required_parameter' && p.type !== 'optional_parameter' && p.type !== 'rest_parameter') continue;
  ...
  if (body) {
    for (let i = 0; i < body.childCount; i++) {
      ...
      if (m.type === 'method_definition') {
        ...
        const paramTypes = [];
        const ptypes = extractParamTypes(params);
        for (const p of ptypes) paramTypes.push(...p.types);
        const returnTypes = extractReturnTypes(retType);
        members.push({
          kind: 'method',
          name: methodName,
          line: m.startPosition.row + 1,
          params: params ? params.namedChildCount : 0
        });
        if (paramTypes.length > 0 || returnTypes.length > 0) {
          methodTypes.push({ method: methodName, paramTypes, returnTypes, line: m.startPosition.row + 1 });
        }
  ...
  if (body) {
    for (let i = 0; i < body.childCount; i++) {
      ...
      if (m.type === 'method_signature') {
        ...
        for (const p of ptypes) paramTypes.push(...p.types);
        const returnTypes = extractReturnTypes(retType);
        members.push({ kind: 'method', name: methodName, line: m.startPosition.row + 1 });
        if (paramTypes.length > 0 || returnTypes.length > 0) {
          methodTypes.push({ method: methodName, paramTypes, returnTypes, line: m.startPosition.row + 1 });
        }
...
function extractFunction(node, entityIdBase, exported) {
  ...
  const methodTypes = [];
  if (paramTypes.length > 0 || returnTypes.length > 0) {
    methodTypes.push({ method: name, paramTypes, returnTypes, line: node.startPosition.row + 1 });
  }
  return {
    kind: 'function',
    name,
    ...
    extends: [],
    implements: [],
    members: [{ kind: 'method', name, params: params ? params.namedChildCount : 0, line: node.startPosition.row + 1 }],
    methodTypes,
    exported
  };
}
...
function extractFile(filePath, root) {
  ...
  for (let i = 0; i < ast.childCount; i++) {
    ...
    if (child.type === 'export_statement') {
      ...
      if (src) {
        result.imports.push({ specifier: src, line: child.startPosition.row + 1, isType: false, isReExport: true, names: [] });
      }
      ...
      const names = getImportedNames(clause);
      if (specifier) {
        result.imports.push({ specifier, line: child.startPosition.row + 1, isType, names });
      }
    } else if (
  ...
  // Also catch CommonJS require() calls anywhere in the file.
  ...
  for (const call of callExprs) {
    ...
    if (fn && fn.text === 'require') {
      ...
      if (str) {
        const spec = str.type === 'string_fragment' ? str.text : unquote(str.text);
        result.imports.push({ specifier: spec, line: call.startPosition.row + 1, isType: false, isCjs: true, names: [] });
      }
    }
  }
