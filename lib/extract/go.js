...
function parseFile(filePath) {
  ...
  try {
    ...
    return { source, ast: tree.rootNode, error: tree.rootNode.hasError ? 'syntax error or unsupported syntax' : null };
  } catch (e) {
...
function extractFile(filePath, root) {
  ...
  extractGoEnumConsts(ast, result, entityBase);
  result.calls = collectCalls(ast, result);
  attachSites(result, ast, 'go');
  return result;
}

function extractGoEnumConsts(ast, result, entityBase) {
  const decls = collectNodes(ast, 'const_declaration');
  const byType = new Map();
  for (const decl of decls) {
    const specs = collectNodes(decl, 'const_spec');
    for (const spec of specs) {
      const nameNode = spec.childForFieldName('name');
      if (!nameNode) continue;
      const typeNode = spec.childForFieldName('type');
      let typeName = typeNode ? typeNode.text.replace(/\s+/g, '') : null;
      if (!typeName || GO_BUILTIN_TYPES.has(typeName)) {
        typeName = spec._goEnumType || null;
      }
      if (!typeName) continue;
      if (!byType.has(typeName)) byType.set(typeName, []);
      byType.get(typeName).push({ kind: 'field', name: nameNode.text, line: spec.startPosition.row + 1 });
    }
    let inherit = null;
    for (const spec of specs) {
      const typeNode = spec.childForFieldName('type');
      if (typeNode && !GO_BUILTIN_TYPES.has(typeNode.text.replace(/\s+/g, ''))) {
        inherit = typeNode.text.replace(/\s+/g, '');
      }
      const nameNode = spec.childForFieldName('name');
      if (!nameNode || !inherit) continue;
      const already = (byType.get(inherit) || []).some((m) => m.name === nameNode.text);
      if (already) continue;
      if (!byType.has(inherit)) byType.set(inherit, []);
      byType.get(inherit).push({ kind: 'field', name: nameNode.text, line: spec.startPosition.row + 1 });
    }
  }
  for (const [typeName, members] of byType) {
    if (members.length < 2) continue;
    const existing = result.entities.find((e) => e.name === typeName);
    if (existing) {
      existing.kind = existing.kind === 'type_alias' ? 'enum' : existing.kind;
      existing.enumLike = true;
      const have = new Set((existing.members || []).map((m) => m.name));
      existing.members = (existing.members || []).concat(members.filter((m) => !have.has(m.name)));
    } else {
      result.entities.push({
        kind: 'enum',
        name: typeName,
        id: entityBase + '#' + typeName,
        line: members[0].line,
        modifiers: [],
        extends: [],
        implements: [],
