...
function resolveViaPyImport(site, fe, ctx) {
  ...
  for (const imp of fe.imports || []) {
    ...
    const bind = bindings.find((b) => b.local === site.callee || b.local === site.object || b.local === name) || null;
    const aliasImported = imp.aliases && (imp.aliases[site.callee] || imp.aliases[site.object] || imp.aliases[name]);
    const hitName = !!(bind || aliasImported || names.includes(site.callee) || names.includes(site.object) || names.includes(name));
    ...
    const importedName = bind
      ? bind.imported
      : (aliasImported || (names.includes(site.callee) ? site.callee : (names.includes(site.object) ? site.object : null)));
    const resolved = ctx.resolvePyModule(imp.specifier, fe, importedName);
    if (!resolved) return { unresolved: true, reason: 'import-missing' };
    const targetFe = ctx.feByFile.get(resolved.file);
    const want = site.object && (hitName || moduleHit)
      ? site.callee
      : ((bind && bind.imported) || aliasImported || site.callee);
...
function changedFilesFromGraphs(base, head) {
  ...
  for (const n of base.nodes || []) {
    if (n.kind === 'file' && n.path) baseHash.set(n.path, n);
  }
  const headPaths = new Set();
  for (const n of head.nodes || []) {
    if (n.kind !== 'file' || !n.path) continue;
    headPaths.add(n.path);
    // Unknown legacy hashes require a conservative call-resolution pass.
    if (compareSourceContentHashes(baseHash.get(n.path), n) !== false) files.add(n.path);
  }
...
function implKeyHash(node, map) {
  if (!node) return;
  const path = node.path || null;
  if (node.impl && node.impl.hash) map.set(node.id, { hash: node.impl.hash, path });
  if (node.behavior && node.behavior.hash) map.set(node.id + '#b', { hash: node.behavior.hash, path });
  for (const [key, impl] of Object.entries(node.methodImpls || {})) {
    if (!impl) continue;
    if (impl.hash) map.set(node.id + '::' + key, { hash: impl.hash, path });
    if (impl.behavior && impl.behavior.hash) {
      map.set(node.id + '::' + key + '#b', { hash: impl.behavior.hash, path });
    }
  }
}

/** Files whose impl / behavior fingerprint changed (throw-only, no structure edge). */
function implTouchedFiles(base, head) {
  const files = new Set();
  const from = new Map();
  const to = new Map();
  for (const n of (base && base.nodes) || []) implKeyHash(n, from);
  for (const n of (head && head.nodes) || []) implKeyHash(n, to);
  for (const [id, hv] of to) {
    const bv = from.get(id);
    if ((!bv || bv.hash !== hv.hash) && hv.path) files.add(hv.path);
  }
  return files;
}

function sourceFingerprintChanged(base, head) {
  if (!base || !head) return false;
  if (base.contentFingerprint && head.contentFingerprint
    && base.contentFingerprint !== head.contentFingerprint) return true;
  return false;
}

function incrementalCallFiles(base, head) {
  const files = changedFilesFromGraphs(base, head);
  for (const p of implTouchedFiles(base, head)) files.add(p);
  return reverseImporters(files, head);
}
...
function attachCallEdges(graph, opts) {
  ...
  if (opts.files) fileSet = new Set(opts.files);
    ...
    fileSet = incrementalCallFiles(opts.base, graph);
    // Structure-only compare can miss throw/impl edits (or old baselines
    // without contentHash). Fall back to a full attach when source changed.
    if (fileSet.size === 0) {
      if (sourceFingerprintChanged(opts.base, graph) || implTouchedFiles(opts.base, graph).size) {
        fileSet = null;
      } else {
        return graph;
      }
