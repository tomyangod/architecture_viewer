...
function importTargets(rel, source, rootDir) {
  const links = [];
  const seen = new Set();
  for (const re of IMPORT_RE) {
    ...
    while ((m = re.exec(source))) {
      ...
      } else if (/^[a-zA-Z_][\w.]*$/.test(spec) && spec.includes('.')) {
        // Python-style package path → file guess
        const asPath = spec.replace(/\./g, '/') + '.py';
        const abs = path.join(rootDir, asPath);
        if (fs.existsSync(abs)) {
          links.push({ target: asPath, confidence: 'medium', via: 'import' });
        }
      }
    }
  }
...
function buildTestIndex(rootDir) {
  const files = [];
  const edges = [];
  for (const rel of walkTestFiles(rootDir)) {
    ...
    const links = [];
    const seen = new Set();
    for (const link of [...importTargets(rel, source, rootDir), ...namingTargets(rel, rootDir)]) {
    ...
    files.push({ path: rel, contentHash: hash, linkCount: links.length, links });
  }
  return {
    version: 1,
...
function fileHashMap(index) {
  const map = new Map();
  for (const f of (index && index.files) || []) {
    if (f.path) map.set(f.path, f.contentHash || null);
  }
  return map;
}

function changedTestFiles(baseIndex, headIndex) {
  const base = fileHashMap(baseIndex);
  const head = fileHashMap(headIndex);
  const changed = new Set();
  for (const [p, h] of head) {
    if (base.get(p) !== h) changed.add(p);
  }
...
/**
 * Production paths covered by tests that changed this session.
 */
