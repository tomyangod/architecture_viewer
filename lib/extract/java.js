      ...
      for (let j = 0; j < child.childCount; j++) {
        ...
        for (let k = 0; k < body.childCount; k++) {
          ...
          if (member.type === 'enum_constant') {
            const nn = member.childForFieldName('name') || findChildByType(member, 'identifier');
            if (nn && nn.text) {
              entity.members.push({ kind: 'field', name: nn.text, line: member.startPosition.row + 1 });
            }
          }
