...
function magicPrompt(_repo) {
  return [
    '【架构检查规则】',
    '用当前工作区根作为项目路径，不要沿用历史对话或 Rules 里的绝对路径（worktree / 多窗口会指错仓）。',
    '在我让你改代码之前，你必须先做一件事：',
    '1. 动手改之前：调用 arch-viewer 的 av_session_start（repo 可省略，默认当前工作区根），拍一张"改之前"的照片；',
    '2. 改完之后：调用 av_session_report 对比前后照片，看有没有改坏结构；',
    '3. 如果报告里有红灯（HIGH），调用 av_explain_finding（from=session）看详情，然后用大白话告诉我：谁串了谁的门、建议怎么改；',
