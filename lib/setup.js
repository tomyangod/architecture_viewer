...
/**
 * Remove only the mcp-arch-viewer insert block; keep comments and other plugins.
 */
function removeDshPatch(patchPath) {
  if (!fs.existsSync(patchPath)) return { changed: false, reason: 'missing' };
  const raw = fs.readFileSync(patchPath, 'utf8');
  if (!raw.includes(DSH_ENTRY_ID)) return { changed: false, reason: 'already' };
  fs.copyFileSync(patchPath, patchPath + '.av-bak');
  const parts = raw.split(/^- insert:/m);
  const header = parts[0] || '';
  const kept = parts.slice(1).filter((p) => !p.includes(DSH_ENTRY_ID));
  let out;
  if (kept.length === 0) {
    const comments = header.split('\n').filter((l) => l.trim() === '' || l.trim().startsWith('#')).join('\n').replace(/\s+$/, '');
    out = (comments ? comments + '\n\n' : '') + '[]\n';
  } else {
    out = header + kept.map((p) => '- insert:' + p).join('');
    if (!out.endsWith('\n')) out += '\n';
  }
...
module.exports = {
  runSetup, magicPrompt, mcpServerPath, serverConfig, targets, readJsonSafe,
  dshRoot, dshProfiles, dshPatchBlock, writeDshPatch, removeDshPatch,
  isGlobalInstalled, globalInstallInfo, resolveLauncher, NPM_PACKAGE, MCP_BIN,
  SERVER_NAME, DSH_ENTRY_ID
};
