...
function buildFileLocMap(diff) {
  ...
  for (const f of diff.fileLocChanges) {
    map[f.path] = { base: f.baseLoc, head: f.headLoc, delta: f.delta, deltaPercent: f.deltaPercent };
  }
  return map;
}
...
.node-badge {
  font-family:var(--mono); font-size:10px; font-weight:700; pointer-events:none;
}
...
function buildLayout(entities, fileLocMap) {
  ...
  return { nodePos, layerBands, fileBoxes, totalWidth, totalHeight: y, fileCount: fileBoxes.length };
}
...
function renderGraph() {
  ...
  document.getElementById('graph-count').textContent =
    visibleEntities.length + ' entities · ' + visibleEdges.length + ' edges · Δ ' + deltaVis.entities.length;
  ...
  const deltaLayout = buildLayout(deltaVis.entities.length ? deltaVis.entities : [], D.fileLocMap);
  ...
  function renderSide(svgEl, side, lay, edges) {
    ...
    const th = empty ? 80 : lay.totalHeight;
    ...
    let html = '';
    if (empty && isDelta) {
      html = \`<text class="layer-label" x="16" y="40">无结构变化</text>\`;
    ...
    for (const box of lay.fileBoxes) {
      const show = isDelta ? box.presence.inDelta : (isBefore ? box.presence.inBefore : box.presence.inAfter);
      if (!show) continue;
      ...
      if (isDelta && box.locInfo && box.locInfo.delta > 0) {
        const sign = box.locInfo.delta > 0 ? '+' : '';
        const pct = box.locInfo.deltaPercent ? \` (\${sign}\${box.locInfo.deltaPercent.toFixed(0)}%)\` : '';
        badges.push(\`<tspan class="fb-loc">\${sign}\${box.locInfo.delta}行\${pct}</tspan>\`);
      }
      if (badges.length) {
    ...
    for (const box of lay.fileBoxes) {
      ...
      for (const p of box.members) {
        ...
        const badge = mark
          ? \`<text class="node-badge \${node.status}" x="\${p.x + p.w - 8}" y="\${p.y + 12}" text-anchor="end">\${mark}</text>\`
          : '';
...
module.exports = { generateReport, buildReportData, assertReportContract: (...a) => require('./report-contract').assertReportContract(...a) };
