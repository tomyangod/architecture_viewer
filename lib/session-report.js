...
.node-rect.moved { stroke:var(--purple); fill:rgba(139,92,246,.08); }
...
<div class="section">
  ...
  <div class="graph-hint">每个方框是一个<strong>文件</strong>，框内是组件。左图 Before、中图 <strong>Delta（仅变化）</strong>、右图 After。Delta 突出 added / removed / modified / moved / rerouted。</div>
  ...
  <div class="triple-graph">
    ...
    <div class="graph-panel delta">
      <div class="graph-panel-title"><span class="tag-label">DELTA</span> 变化视图</div>
...
function renderGraph() {
  ...
  const layout = buildLayout(visibleEntities);
  const deltaLayout = buildLayout(deltaVis.entities.length ? deltaVis.entities : []);
