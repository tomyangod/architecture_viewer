 ...
 */
function generateReport({ baseGraph, headGraph, diff, findings, repoName, sessionStart }) {
  const payload = buildReportData(baseGraph, headGraph, diff, findings, repoName, sessionStart);
 ...
 */
function buildReportData(baseGraph, headGraph, headDiff, findings, repoName, sessionStart) {
  ...
  const removedIds = new Set(headDiff.removedNodes.map((n) => n.id));
  ...
  for (const n of headGraph.nodes) {
    ...
    entityMap.set(n.id, {
      ...
      path: n.path,
      methods: n.methods || [],
      status: addedIds.has(n.id) ? 'added' : 'unchanged'
    });
  }
  for (const n of baseGraph.nodes) {
    if (!TYPE_KINDS.includes(n.kind)) continue;
    if (entityMap.has(n.id)) {
      if (removedIds.has(n.id)) entityMap.get(n.id).status = 'removed';
    } else {
      entityMap.set(n.id, {
        id: n.id,
        name: n.name,
...
<div class="section">
  <div class="section-title">架构变更图 <span class="count" id="graph-count"></span></div>
  <div class="filter-bar">
    <button class="filter-btn active" data-filter="all">全部</button>
    <button class="filter-btn" data-filter="changed">仅变更</button>
  ...
  <div class="graph-container">
    ...
    <div class="graph-legend">
      ...
      <div class="legend-item"><span class="legend-swatch" style="background:var(--green)"></span> 新增关系</div>
      <div class="legend-item"><span class="legend-swatch" style="background:var(--red);opacity:.5"></span> 删除关系</div>
      <div class="legend-item"><span class="legend-swatch" style="background:var(--orange)"></span> 违规</div>
    </div>
  </div>
</div>
...
const NODE_W = 130, NODE_H = 36, NODE_GAP_X = 16, NODE_GAP_Y = 14;
const LAYER_PAD = 30, LAYER_LABEL_W = 80;
...
function renderGraph() {
  ...
  if (currentFilter === 'changed') {
    const changedIds = new Set(entities.filter(e => e.status !== 'unchanged').map(e => e.id));
    edges.forEach(e => { if (e.status !== 'unchanged' || e.violation) { changedIds.add(e.from); changedIds.add(e.to); } });
    visibleEntities = entities.filter(e => changedIds.has(e.id));
    visibleEdges = edges.filter(e => changedIds.has(e.from) && changedIds.has(e.to) && (e.status !== 'unchanged' || e.violation));
  } else if (currentFilter === 'violations') {
    const violIds = new Set();
    edges.forEach(e => { if (e.violation) { violIds.add(e.from); violIds.add(e.to); } });
    visibleEntities = entities.filter(e => violIds.has(e.id));
    visibleEdges = edges.filter(e => e.violation);
  }

  // Group by layer
  const byLayer = {};
  for (const layer of D.layers) byLayer[layer] = [];
  ...
  const layerRects = [];
  const maxPerRow = Math.max(1, ...Object.values(byLayer).map(g => g.length));
  const totalWidth = LAYER_LABEL_W + maxPerRow * (NODE_W + NODE_GAP_X) + LAYER_PAD;

  for (const layer of [...D.layers, '未分类']) {
    const group = byLayer[layer];
    if (!group || group.length === 0) continue;
    const rowH = NODE_H + NODE_GAP_Y;
    const bandH = rowH + 16;
    layerRects.push({ layer, y, h: bandH, count: group.length });

    const startX = LAYER_LABEL_W + 10;
    group.forEach((node, i) => {
      const nx = startX + i * (NODE_W + NODE_GAP_X);
      nodePos.set(node.id, { x: nx, y: y + 8, w: NODE_W, h: NODE_H, node });
    });
    y += bandH;
  }

  const totalHeight = y + LAYER_PAD;
  svg.setAttribute('viewBox', \`0 0 \${totalWidth} \${totalHeight}\`);
  svg.setAttribute('width', totalWidth);
  svg.setAttribute('height', totalHeight);
  document.getElementById('graph-count').textContent = visibleEntities.length + ' entities · ' + visibleEdges.length + ' edges';

  let html = '';

  // Layer bands
  for (const lr of layerRects) {
    const color = layerColor(lr.layer);
    html += \`<rect class="layer-band" x="0" y="\${lr.y}" width="\${totalWidth}" height="\${lr.h}" rx="8"/>\`;
    html += \`<text class="layer-label" x="10" y="\${lr.y + lr.h/2 + 4}">\${layerLabelCn(lr.layer)}</text>\`;
    html += \`<line x1="\${LAYER_LABEL_W - 5}" y1="\${lr.y + 4}" x2="\${LAYER_LABEL_W - 5}" y2="\${lr.y + lr.h - 4}" stroke="\${color}" stroke-width="2" opacity=".3" stroke-linecap="round"/>\`;
  }
  ...
  for (const edge of visibleEdges) {
    ...
    html += \`<path class="edge-path \${cls}" d="\${path}" data-edge="\${edge.from}|\${edge.type}|\${edge.to}"/>\`;
  }

  // Nodes
  for (const [id, p] of nodePos) {
    const node = p.node;
    const hasViolation = visibleEdges.some(e => e.violation && (e.from === id || e.to === id));
    let cls = node.status;
    if (hasViolation) cls = 'violation';
    const color = layerColor(node.layer);
    html += \`<g class="node-group" data-id="\${id}">
      <rect class="node-rect \${cls}" x="\${p.x}" y="\${p.y}" width="\${p.w}" height="\${p.h}"/>
      <text class="node-kind" x="\${p.x + 8}" y="\${p.y + 13}">\${node.kind}</text>
      <text class="node-text \${node.status}" x="\${p.x + 8}" y="\${p.y + 27}">\${truncate(node.name, 18)}</text>
    </g>\`;
  }
  ...
  svg.querySelectorAll('.node-group').forEach(g => {
    g.addEventListener('mouseenter', (ev) => {
      ...
      svg.querySelectorAll('.edge-path').forEach(p => {
        p.style.opacity = p.dataset.edge && p.dataset.edge.startsWith(id+'|') ? '1' : '';
      });
    });
    ...
    g.addEventListener('mouseleave', () => {
      ...
      svg.querySelectorAll('.edge-path').forEach(p => p.style.opacity = '';
    });
  });
}
...
let entHtml = '<div class="section-title">实体变更 <span class="count">+' + added.length + ' -' + removed.length + ' ~' + D.modified.length + '</span></div>';
    ...
    entHtml += \`<div class="change-item" style="flex-wrap:wrap">
      ...
      <div style="width:100%">
        \${m.changes.map(c => {
          if (c.field === 'methods') {
            let h = '';
            if (c.added) h += '<div class="change-detail"><span class="plus">+ 方法: ' + c.added.join(', ') + '</span></div>';
            if (c.removed) h += '<div class="change-detail"><span class="minus">- 方法: ' + c.removed.join(', ') + '</span></div>';
            return h;
          }
...
if (!added.length && !removed.length && !D.modified.length) {
