 ...
 */
function generateReport({ baseGraph, headGraph, diff, findings, impact, repoName, sessionStart, analyzerStatus }) {
...
function buildSessionReportJson({ diff, findings, riskSummary, impact, baseGraph, headGraph, repoName, analyzerStatus }) {
  const list = findings || [];
  const risk = {
  ...
  if (canView) {
    const view = buildReportData(
      ...
      payload.repo || 'repo', payload.sessionStart
    );
