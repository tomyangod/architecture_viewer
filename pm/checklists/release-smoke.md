...
> 候选版：0.12.2-rc.2（待发布）。每项记录版本、执行时间、命令和结果；未执行写 NOT RUN，不用本地模拟替代远端验收。
...
| 远端验证 | npm 发布/registry 安装、真实公网 PR、客户接入均 **NOT RUN** |
...
- [ ] 全新目录安装 tarball，安装后 CLI `--version` 为 0.12.2-rc.2。
