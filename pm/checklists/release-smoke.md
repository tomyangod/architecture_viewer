...
> 当前候选：0.12.2-rc.5（解释闭环 / 分层门禁解耦；发布记录见 docs/commercial/npm-publish.md）。rc.4（默认交付收缩）与 rc.3（81e6c43）已于 2026-09-14 发布。历史 rc.2 本地验收记录保留在下方，勿改写。每项记录版本、执行时间、命令和结果；未执行写 NOT RUN，不用本地模拟替代远端验收。
