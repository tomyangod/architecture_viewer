...
- registry 查询：`latest=0.12.1`、`next=0.12.2-rc.1`。
- 本地候选：`0.12.2-rc.2`，尚未提交或发布；不记为客户安装/激活。
- 审查发现的 3 项交付阻塞及复核补充的目录遍历/YAML 兼容问题已修复；源码发布校验 840 通过/0 失败/1 跳过，最终重打包后的全新安装回归 223/223。证据见 [发布冒烟记录](checklists/release-smoke.md)，不能替代客户验证。
- 真实托管 PR、registry 候选包安装及外部团队接入尚未核验，不从本地自动化推断成功。
