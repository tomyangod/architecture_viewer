...
| **触达数** | 当周新发出的个性化触达条数（软评/私信/Issue/Discussion；不含重复刷同一对象；跳过不计） | [p1-leads](growth/p1-leads-2026-09-12.md) + [outreach-batch](growth/outreach-batch-2026-09-14.md) 登记表 | 每周五 |
| **回复数** | 当周收到的有效回复条数（已读未回不计；系统自动回复不计） | 同上 + 触达回复巡检 | 每周五 |
| **访谈数** | 当周完成的 15 分钟筛访谈场次（已预约未开场不计） | seed-users / 线索表「已访谈」 | 每周五 |
| **接入数** | 当周新接入试点团队数（完成 onboard checklist 首周旁路登记） | [p1-onboard-checklist](growth/p1-onboard-checklist.md) | 每周五 |
...
| W08（09/13–09/19） |  |  |  |  |  |  |  |  | 阶段 B：招募；**先行指标见「获客先行指标」表**（触达/回复/访谈/接入） |
| W09（09/20–09/26） |  |  |  |  |  |  |  |  | |
| W10（09/27–10/03） |  |  |  |  |  |  |  |  | |
...
### 获客先行指标（阶段 B · 不等 10-09 才发现招募失败）

> 目的：把「触达 → 回复 → 访谈 → 接入」做成**每周五必刷**的先行指标，避免只盯安装/付费而漏掉招募窗口。  
> 台账纪律：未测量写 **未知**，已测量为零写 **0**；同一对象重复触达不计入触达数。口径源见上表。

| 周次 | 触达数 | 回复数 | 访谈数 | 接入数 | 备注 |
|---|---|---|---|---|---|
| W08（09/13–09/19） |  |  |  |  | 周五回刷；期间累计见下 |
| W09（09/20–09/26） |  |  |  |  | 最晚接入窗口相关 |
| W10（09/27–10/03） |  |  |  |  | |
| W11（10/04–10/10） |  |  |  |  | **10-09 检查点：≥3 团队主动留存（满 28 天台账）** |

**期间快照（2026-09-14，非完整周，人工）**

| 项 | 值 | 依据 |
|---|---|---|
| 本周新触达（私信/第二波 GitHub） | **9** | 掘金 A1–A6 + GitHub B1–B3；见 outreach-batch 登记 |
| 历史软评存量（不计入本周「新触达」分子时请分列） | 已发 | 掘金 #1–#6/#16–#18、知乎 #19–#23、GitHub #10–#12/#24/#26–#30 等；见 p1-leads |
| 回复数 | **0** | 截至快照无有效回复 |
| 访谈数 | **0** | |
| 接入数 | **0** | |
...
- **待发布**：`arch-viewer@0.12.2-rc.7` → npm 标签 `next`；`latest` 保持 `0.12.1`。
- 源提交：`2f4d5b7`（稀疏敏感面卡片与肖像 #22、Review Walk 功能 #20、R18 枚举完备性与 R19 异常漂移 #21）。
- SHA-256：**待发布后回填**。
- registry shasum：**待发布后回填**。
- integrity：**待发布后回填**。
- registry 全新安装冒烟：**待发布后回填**。
- 发布前全量单测：**待发布后回填**。
...
- 已发布：`arch-viewer@0.12.2-rc.5` → 当时 npm 标签 `next`；`latest` 保持 `0.12.1`。`next` 已于同日下午改指 rc.6。
