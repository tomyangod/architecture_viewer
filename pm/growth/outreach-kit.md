...
| 项 | 准确说法 | 不能说 |
|---|---|---|
| 版本 | 试点钉 `arch-viewer@0.12.2-rc.4`（npm `next`） | "装 latest 就行"（latest 是 0.12.1，缺本轮修复） |
...
2. 版本统一用 0.12.2-rc.4：npx --yes --package arch-viewer@0.12.2-rc.4 arch-viewer session report <仓库路径>
...
· 做什么：在你们一个真实仓库上，用固定版本 0.12.2-rc.4 对照 git HEAD
