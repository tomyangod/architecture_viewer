...
sample-shop/
├── .av/
│   └── layers.json                  # 分层配置（rc.6 必需，确保跨层检测为阻断级）
...
分层配置文件（rc.6 必需，确保跨层违规为阻断级 HIGH）：
...
```bash
# 0. 前置：Node ≥ 18、git
# 1. 建仓并创建 .av/layers.json（内容见上，确保跨层检测为阻断级）
#    和 app/ 目录下的三个文件（controller/service/storage，最小实现即可）
mkdir -p sample-shop/.av sample-shop/app/controllers sample-shop/app/services sample-shop/app/database
cd sample-shop
# 将上面的 layers.json 内容保存到 .av/layers.json
# 将上面的 Python 代码保存到对应文件
