...
> **本地版交付边界：** 六视图、CLI/MCP 结构变更报告与人工核实的风险提示。绿灯不保证没有漏报；分析不完整不是通过。支持范围、离线模式与已知局限见随包交付的 [首发交付范围](docs/delivery-scope.md)。AI 精修、扩展、多仓和 SaaS 不在首发质量承诺内。

> **当前分支：`0.12.2-rc.3`，尚未发布。** 本次整合请使用对应提交打出的 tarball 验收，不以 `npx` 下载的 `latest` / `next` 作为同一实现的证据；下文通用安装命令面向已发布包。RC2 未包含此前独立 RC1 加固，版本号相邻不代表修复已继承。
