...
> **风格参考**：[MCP-DEMO-publicopinionmonitor.md](./MCP-DEMO-publicopinionmonitor.md)（舆情仓实战）；本篇为「任意新项目」通用 Quickstart，含功能全景与报告解读。
...
它**看不见**业务语义与运行时行为——下列问题**不是** session / drift 的目标，请用测试、审查或专门静态分析：
...
**管什么（会亮灯）**：跨层串门、层级穿透、类型删除、新第三方依赖、文件级循环 import、神文件/行数暴涨、新模块未上图（漂移）等——见 §4.3。

一句话：**红灯 = 结构可能坏了；绿灯 ≠ 代码写对了。**
...
| 情况 | 行为 |
|------|------|
| 无 `.av/graph-baseline.json` | 跳过（先 `session start`） |
| 有基线且无 HIGH | 放行 |
| 有 HIGH 红灯 | **拒绝提交**；看 `.av/session-report.builtin.html` |
...
**不要写死绝对路径。** `repo` 可省略，默认当前工作区根；worktree / 多窗口沿用主仓路径会拍错照片。

```text
对当前工作区：
1. 动手改代码前：必须先 av_session_start（repo 可省略；不要沿用历史绝对路径）。
2. 改完准备收工：先 av_session_changes；有变更再 av_session_report。
3. 有红灯：av_explain_finding（from=session）用人话说明谁串了谁、跳了哪层、怎么改；修完再 report。找不到本轮 finding 时不要改扫全楼。
...
2. 打开 `$REPO` 后对话：「对当前工作区执行 `av_session_start`」（不要贴死主仓绝对路径）  
