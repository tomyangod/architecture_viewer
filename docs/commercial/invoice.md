...
| 档位 | 价格 | 开票内容建议 |
|------|------|--------------|
| Pro | ¥29 / 月 / 账号（或 ¥199 / 年） | 软件服务费 / 技术服务费 |
| Team | 入门包 **¥4,999 / 年**（5 席）；或 **¥99 / 人 / 月**（验证期人工报价，无自助下单） | 同上；仓库 URL 选填 |
| AI 改码架构体检 | 早鸟 **¥999 / 次**（至 2026-09-30，限 3 席）；其后 **¥1,999 / 次** | 技术服务费；档位写「架构体检」；私有仓不要把 URL 写进公开 Issue |
...
| 档位 | 是 | Pro 月付 / Team 年付 / 架构体检（仓库 HTTPS URL 选填） |
| 开通时长 | 是 | 月数或年数 |
| 公司抬头 | 是 | 与营业执照一致 |
| 纳税人识别号 | 是 | 统一社会信用代码 |
| 开户行 + 账号 | 专票必填 | 普票可空 |
...
2. 按上表提交开票信息 + 转账（备注 `AV-Pro` / `AV-Team` / `AV-Diagnostic` + 注册邮箱；Team 若有 orderId 一并写上）。
3. 运营核验到账后开通：
