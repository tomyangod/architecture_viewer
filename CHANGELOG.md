...
试点候选包，拟发布到 npm 标签 `next`（不改 `latest`）。在 rc.5 解释闭环与分层门禁解耦之上，修正出图页脚夸大校验范围、环路径消歧与 explain 结构化证据。
...
- **风险计数口径**：verdict 的总数与分项均包含 INFO；展示修正不改变阻断阈值。
- **未选中图的保留**：不再按占位词删除图，仅清理与当前随包模板逐字节一致的文件；编辑图、未知旧模板和符号链接保留，缓存命中与精修使用相同规则。清理或生成回执写入失败显式报错。
- **换行归因**：源码、schema 与测试内容哈希统一规范化 CRLF → LF；真实内容变化仍计入。旧 HEAD 缓存重建，无法比较的旧快照哈希显式标记未知，不把哈希格式升级当成代码修改。
- **Python 测试关联**：按 AST 与现有模块解析器识别 `from pkg import submodule`、别名、多行和相对导入，排除注释/字符串伪导入。行为 finding 区分关联测试改动、未改动、未识别和无法比较，不把静态证据缺失说成没有测试。
...
- 区分专用 Archify 导出的人工确认与 `session report --renderer archify|auto` 的显式渲染器选择；两者均不证明架构关系真实。
...
- **交付分支整合**：保留 RC2 的显式排除、范围变化与严格规则校验，同时纳入 RC1 的完整性门、限定名/别名绑定和安装包回归。
- **完整性边界**：SQL/Prisma 文件不可读、单目录内文件上限及持久化不完整基线不能产生正常验收结论。
- **依赖证据**：JS/TS 未绑定类型与缺失本地导出记录为未解析引用，不创建悬空外部边；再导出不充当局部绑定。保留显式本地和第三方别名真边。
- **排除规则**：带已知路径前缀的否定规则不会重新放开无关浏览器缓存目录；精修取证与缓存遵循同一排除范围。
- **生成缓存**：核对输入内容、配置、生成实现、引擎与实际输出字节；占位稿、旧校验和部分精修不能冒充完整缓存命中。
- **精修提示**：移除固定示例业务、框架与数据库假设；图协议通过不等于运行时语义已验证。
- **六视图事实化**：移除按库存顺序虚构的关系、使用者和占位类型；保留有来源的导入、继承与显式配置关系。语言按模块识别，调度/监控/告警源码可见。HTML 直接展示人工核实提示，不再仅依赖 Markdown 草稿说明。

### Added

- **快照与版本取证**：`av_check_layering` 单独列出现有依赖/体量/孤立实体库存，不当成本轮新增或分层违规；MCP 返回实际包路径、版本、分析源码摘要和扫描时间。
- 已生成六视图的离线浏览器回归，以及安装包内的解析绑定与缓存回归。测试不调用付费模型，不能作为模型输出准确性的证明。
...
## \[0.12.2-rc.1\] — 2026-09-11

独立交付加固分支的本地人工评审候选，未包含于 RC2 发布提交，不是稳定商用发布。平台通过状态以对应提交的实际验收结果为准。

### Fixed

- **分析完整性**：空扫、解析失败、扫描截断、不可读目录和无效分层配置不再给出假绿灯；会话 CLI/MCP 使用退出码 3 区分分析不可用与风险违规。JSON/HTML 和 MCP 轮询保留分析状态与原因。
- **扫描范围**：不再误排除 Java 的 `com/example` 包；根 `.gitignore` 字面目录规则保留锚定和顺序，`/runtime/` 不再误排除 `tools/runtime/`。
- **Python 依赖证据**：保留导入别名与限定名，解析显式再导出到实际定义；未解析引用不再伪装成本地实体或第三方包。取消 Py/JS 全仓同名猜测，保留显式导入绑定。
- **语法与规则失败**：tree-sitter 检出的语法错误计入不完整；MCP 团队规则加载失败不再静默回退。

### Added

- 随 npm 包交付的 `docs/delivery-scope.md`，明确本地人工评审范围、离线模式、平台验收与解析局限。
- 干净安装包 CLI/MCP/HTML 验收脚本及 OS/Node CI 矩阵；矩阵配置本身不代表对应平台已通过。
